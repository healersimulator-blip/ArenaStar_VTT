...
**Remaining P09.** Strategic weapon-state columns (M01), the per-target DC 12 Reflex burst save rolls (save pipeline, not the DC constant), and a dedicated `e2e/pf1e_firearms.spec.ts` (load provokes AoO → ammo 1→0 → reload → 1, Quick Clear move vs standard, Expert Loading saves grit, explosion burst highlight) — then P09 is done and M03's tactical consumers are closed.
