...
**Evidence, all executed.** `corepack pnpm typecheck` exit 0 · `corepack pnpm lint` exit 0 · `corepack pnpm test` **217 files / 2,482 passed / 5 skipped** (new: `tests/host/worldFilePackages.test.ts` 7, `tests/host/zipKind.test.ts` 8, `tests/app/hostPackages.test.ts` 4, `packageLoader.test.ts` +2; `worldFile.test.ts` asserts format 2) · `corepack pnpm build` **2,668,468 B raw / 762,442 B gzip**, `size` OK within the 6 MB budget · `node scripts/coverage.mjs --check` OK. `e2e/worldfile.spec.ts` updated for `meta.format === 2` + empty `packages.json`; Playwright's Chromium could not be downloaded in this sandbox (CDN unreachable, D-222 precedent), so the browser matrix is to be re-run by the next environment that has it — the affected specs (`worldfile`, `packages`, `gmextras`, `pf1e_mass_battles`) keep every selector and value they use.
