...
Two user-requested tactical/strategic features that sit on top of the landed P0–P09 stack. Both reuse the existing host-authoritative Op/Msg/ModelPool contracts; neither invents a second rule engine. `combat_resolver_5.html` was attached to the prompt but is not present in the workspace snapshot — the strategic spec below is reverse-engineered from the user's description plus the Combat_Resolver_5 parity notes in M Tasks 4–7 and `massBattlePf1e.ts`'s current sequential resolver. When the file is available the phase names / damage-order tie-breakers below should be reconciled verbatim against it before encoding.
...
5. **Combat_Resolver_5 reconciliation.** When `combat_resolver_5.html` is available, diff its phase names (`move`/`shoot`/`melee`/`morale`) and its damage-order tie-breakers against the above; transcribe any verbatim `initiativeModifier + d20` wording into `src/packages/pf1e/rollData.ts:initiativeRoll`'s tie path before marking F02 landed. No code in this slice invents a second initiative formula.
...
* **Open frontier for F-items:** only `combat_resolver_5.html` verbatim tie-breaker reconciliation (per D-220) and the cross-browser firefox/webkit acceptance (chromium-only here) remain for fully marking F01–F03 landed.
* See D-221 and D-222 in `DECISIONS.md` for the full rationale, the rejected alternatives (tolerant inverses, offline client RNG, weakened assertions), and the six-step e2e recipe (replaces D-020's stale toolchain paths).
...
6. Reconcile **Combat_Resolver_5** (`combat_resolver_5.html` — re-attach when available) phase names / damage-order tie-breakers verbatim against F02 before marking it landed.
