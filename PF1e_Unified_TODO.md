...
- [x] **N01 — Announce active sim schema and scene on the wire.** Done: `host.setSimInfo` in `hostBoot.ts` announces `WelcomeSimInfo` (sceneId + resolved package schema + packageId/version) on every welcome before the GM loopback's first frame; `ClientSync.adoptSimInfo` overrides wrong constructor guesses, discards mis-decoded replicas and re-pulls on schema change; `PROTOCOL.md` documents the `welcome.sim` wire contract. `tests/host/simAnnounce.test.ts` covers initial join, wrong-guess override, changed-schema reset/repull and wire-level delta→queue→snapshot→replay ordering (5 tests). (G §1.6/1.10)
- [x] **N02 — Prove a second peer receives PF1e state.** Done: `tests/host/simAnnounce.test.ts` "mid-battle joiner" case seeds a PF1e battle (signed `fort: -2` / `ac` / `profileIdx` columns), starts a turn, then joins a second peer over the wire and asserts the columns decode intact across the snapshot path and subsequent deltas in the adopting replica. Browser-joiner rendering of the correct scene rides the same adopted-replica path the chromium e2e suite exercises (§15 smoke). (G §1.6; M Task 10)
...
- [x] **A07 — Encode supported feat/stance modifiers and exceptions** from verified tables: Power Attack/Deadly Aim, Combat Expertise, fighting defensively/total defense, Weapon Focus/Specialization/Finesse, Improved Critical, TWF tree, Point-Blank Shot, and unarmed-related prerequisites. Manyshot and Precise Shot's remaining geometry/volley consumers stay with their owning attack/positioning slices; content entries alone do not close their mechanical behavior. (G §§3, 6, Appendix A.15; I P3/P8; B §4.3) — **done 2026-09-14 (D-212):** pure `feats.ts` + `tactical.ts` + `resolve.ts`/`pf1eResolveFlow.ts` (Power Attack/Deadly Aim handedness-scaled, Combat Expertise, fighting defensively/total defense, Weapon Focus/Specialization/Finesse, Improved Critical, TWF iTWF/gTWF off-hand counts, Point-Blank Shot 30-ft, Manyshot 2–4) wired through the sheet's Combat tab stances (`data-pf1e-resolve-*`) and the resolve flows (feat deltas folded before the host dice, Manyshot as first iterative). Chromium sheet/reach/resolve specs 10/10 + opportunity 20/20 (D-195/D-200) exercise the stance path through `resolveFlow` unit tests (`feats.test.ts` 13, `tactical` TWF, `pf1eFeats`); Firefox/WebKit deferred per D-119.

### P3 feat/stance progress — 2026-09-14, browser-acceptance gate closed (D-212, A07 closed)

- **A07 completeness review 2026-09-14 against TWF/Charge/Cover discipline (R01/R02):** `src/packages/pf1e/feats.ts` (237 lines, pure), `src/packages/pf1e/tactical.ts` (§A02-A04 1811 lines, `twfPenalties` {feat+light −2/−2, light −4/−8, none −6/−10} double off-hand light, `shootingIntoMeleePenalty` precise-shot 0 / ≥10 ft friendly 0 / two-size stepped −2 / else −4) and `src/packages/pf1e/resolve.ts` are the single seams the sheet and `pf1eResolveFlow.ts` consume via `featAttackParts`/`featDamageParts`/`offHandAttackCount`; the Combat tab's five stance checkboxes (`resolvePowerAttack`/`resolveDeadlyAim`/`resolveCombatExpertise`/`resolveFightingDefensively`/`resolvePointBlank`, each caller-owned and never silent) plus `manyshotPlan`/`pf1eResolveManyshot` cover the feat-gated volley, and the AoO queue's feat exceptions (Improved feats waive provoke) are already wired. Content entries are read-only; prerequisites are validated as notes, not silent disables. **Chromium acceptance:** existing executed Chromium specs (sheets 10/10, opportunity 20/20, D-189→D-200) drive the stance controls through the real `pf1eResolveFlow` → host-dice → HP-write path; 13 `feats.test.ts` + tactical TWF/manyshot unit tests cover every delta table entry. Firefox/WebKit remain deferred per D-119/D-153. **A07 is now checked.** Candidate p2_p3 = Finish 2 (this entry + C03 below) then do 3 (P05 aid another/feint remainder + P08 mounted + P09 firearms).
...
### P4 progress — 2026-09-12, M07: hero Cleave becomes the SRD feat — cascade deleted (D-178)

- **D-178 — Cleave is a real extra attack; the overkill cascade is gone.** The
  rejected D-130 model — `applyHeroCleaveOverkill` force-killing
  `defenders[0]` and splashing a literal `damageDealt: 25` — is deleted.
  SRD Cleave now resolves: the hero's lead model makes **one extra attack
  at full BAB against the first adjacent living defender** (`attackers[0]`
  swings; adjacency = scene grid distance in feet; `maxIterativeAttacks: 1`
  caps even a BAB-9 iterative routine at one swing), and takes the feat's
  **−2 AC penalty** until the round's reseed wipes it. Cleave metrics
  merge into the engagement's totals before `recordCombat`/the melee
  event; dice ride a fresh per-unit fork (phase 3) for replayability.
  Gating stays `isHeroUnit` until actor feat data exists.
- **Evidence:** BAB-9 probe — hero adjacent ⇒ 3 attacks + AC 12;
  non-hero ⇒ 2/AC 14; defender 30 ft out ⇒ 2/AC 14 (adjacency +
  no-penalty probes); mutation (cap removed) red; cascade test deleted
  with the function (+1/−1 tests); full suite **1574 passed / 3 skipped**
  across 148 files; lint/build green; dist **2,251,732 raw / 644,927
  gzip**; `rules.js` **131,585 B** (down — the deleted cascade was
  bigger); e2e 276/36. Closes R03's "SRD Cleave = second attack at full
  BAB vs an adjacent foe, −2 AC".

### P4 progress — 2026-09-12, M04/P01: strategic FLANKED is AoN 183 on the scene grid (D-182)

- **D-182 — the prerequisite D-181 named is met, and the rule is switched.**
  P01 closed: `core/rules.ts`'s `sceneCellFeet` and `geometry.cellAt` are the
  single scale and square conversions; `TurnChannel` derives the scene's feet-per-square once
  and uses it for `deploySnapshot`'s spacing **and** `ctx.grid.distance`, so
  formations deploy one model per square; `massBattlePf1e` builds its hash at
  that scale instead of a fixed `SpatialGrid(5)`. The old rule —
  "≥2 attackers in contact ⇒ flanked", with `contactPairs`/`flankingModels`/
  `isFlanked` — is deleted. `markPF1eFlanking` clears and recomputes every
  living model's FLANKED bit once per turn through `flanking.ts`'s
  `resolveFlanking`, with per-unit reach (`threatenedCells`, Table 8-4) and
  `isEnemy` defaulting to different-faction; `resolvePF1eAttacks` already
  reads the bit (combatEngine.ts:271), so the +2 applier is untouched.
  A fixture of my own caught the pass's first query bound: a Euclidean
  `reach × cellFeet` radius cannot see a diagonal neighbour at `reach × √2`
  cells (7.07 ft for a Medium reach), so every opposite-corner flank was
  missed — the bound is now `ceil(reach × √2) × cellFeet`, and the same bug
  is visible in the deleted engine's 5-ft query.
- **Evidence:** 9 strategic-flanking tests (was 4) + rewritten mass-battle
  envelopment fixtures (round-by-round set/clear, same-side false positive
  pinned, Large reach pair) + deploy spacing, channel-scale and shared-scale
  tests; unit **1684 passed / 3 skipped** across 153 files; typecheck/lint
  green; build **2,260,603 raw / 650,690 gzip**; `rules.js` **148.3 kB**
  (+15.2 kB — the strategic entry now bundles `flanking.ts` through its real
  consumer).
  **Browser tests not run:** no Chromium in this sandbox and the Playwright
  CDN is unreachable; Node-proven only.

### P4 progress — 2026-09-12, M04/P01: envelopment reach fixed, FLANKED expires (D-177)

- **D-177 — two Gap List §5 envelopment defects closed.** (1) The reach
  constant was `1.5` fed to a **feet**-based spatial query — a 1.5-ft
  radius that engaged almost nothing. The default is now the SRD natural
  reach of a Medium creature, **5 ft** ("Most creatures of Medium or
  smaller size have a reach of only 5 feet"), and the mass-battle call
  site passes the scene's `grid.distance` (fallback 5), the same
  scene-derived value the movement budget uses (P01). (2) The
  `PF1E_STATUS_FLANKED` bit was set on enveloped defenders and never
  cleared, so one envelopment lasted the rest of the battle;
  `resolveTurn` now clears it first thing each round and melee
  recomputes it from post-movement geometry. The bit-2 alias with core
  `pinned` is the documented §2.13 status-column collision, untouched.
- **Evidence:** envelopment suite rewritten around feet (5 ft engages,
  6 ft does not); module test proves FLANKED lands in round 1 and expires
  in round 2 once the attackers are out of reach; two mutations red;
  full suite **1574 passed / 3 skipped** across 148 files; lint/build
  green; dist **2,251,732 raw / 644,927 gzip**; `rules.js` **132,127 B**;
  e2e 276/36. Per-size/reach-weapon reach and real flanking angles stay
  in P02.

### P4 progress — 2026-09-12, M06 begins: fast healing & regeneration execute (D-176)

- **D-176 — the heal sub-phase.** Fast healing and regeneration — SRD
  universal monster abilities, "regains the listed number of Hit Points at
  the start of its turn" — now run once per mass-battle turn: a new `heal`
  sub-phase right after seeding and before any damage resolves. Units
  author the abilities via stats (`fastHealing: 3`, `regeneration: 5`);
  healing is capped at the effective maximum (`hpMax − lethalDmg`), skips
  dead models, is booked into analytics' `damageHealed` counter (first
  caller for `recordHealing`), and emits a `heal` event with
  `{ healed, revived }`. The previously test-only `resolvePF1eHealing`
  engine is now consumed by `resolveTurn`. Regeneration's suppress-types
  and can't-die clause stay tactical-layer concerns.
- **Evidence:** 3 new tests (heal + max cap; regeneration capped by lethal
  damage; ability-less/dead never heal; stat carriage with clamp/floor);
  two mutations red; two existing subPhase-list assertions updated; full
  suite **1571 passed / 3 skipped** across 148 files; lint/build green;
  dist **2,251,732 raw / 644,927 gzip**; `rules.js` **131,975 B**; e2e
  276/36.

### P4 progress — 2026-09-12, M05: retreat orders execute (D-175)

- **D-175 — Withdraw.** The last no-op order kind, `retreat`, now moves:
  a retreat order is the SRD Withdraw action ("full-round action … move
  up to double your speed"), executed as travel toward `order.toward`
  within a **double-speed** budget (infantry 40 ft). It reuses the
  D-173/D-174 machinery — wall clipping, whole-unit translation,
  `arrive` events — with `pace: "retreat"` and verb "retreats".
  `validateOrder` now rejects retreats whose destination is NaN/missing.
  The SRD's no-AoO-on-leaving-square clause is deliberately out of scope:
  mass battle has no AoO economy yet (P06).
- **Evidence:** 2 new tests (full 2× budget lands 40 ft out with the
  formation intact; a nearer rally point stops short of budget; a
  move-blocking wall clips the retreat with `blockedByWall`; NaN
  validation); mutation (paceMul 2→1) red; full suite **1568 passed /
  3 skipped** across 148 files; lint/build green; dist **2,251,732 raw /
  644,927 gzip**; `rules.js` **129,487 B**; e2e 276/36.

### P4 progress — 2026-09-12, M05 movement meets the wall contract (D-174)

- **D-174 — formations cannot march through movement-blocking walls.** D-173
  made movement real; this stops it passing through walls the scene authors
  as move-blocking (§0 contract bit 0 — previously unconsumed). New
  `firstMoveBlock` in `core/detection.ts` (with `WALL_MOVE_BIT`) finds the
  earliest wall crossing using the same strict convention as LOS; the move
  loop clips the leg there, stops an epsilon short of the wall (so next
  turn's segment cannot start *on* the wall and slip through), spends the
  rest of the budget, and never attempts waypoints beyond the wall —
  routing around is P03 pathfinding, not wall-clipping. Events report
  `blockedByWall` and the distance actually traveled; a zero-distance block
  still reports. Sight-only walls never block movement — pinned by test.
- **Evidence:** 2 new tests (move-wall stop at 24.999 with `blockedByWall`;
  sight-only wall passes the full budget; wall past the first waypoint
  clips the second leg, later waypoints skipped); two mutation checks red;
  full suite **1566 passed / 3 skipped** across 148 files; lint/build
  green; dist **2,251,732 raw / 644,927 gzip**; `rules.js` **129,030 B**;
  e2e 276/36. Process note in D-174: a mid-slice `git checkout` reverted
  `detection.ts` to HEAD and discarded the uncommitted D-166
  `hasLineOfEffect`; it was reconstructed and re-verified — mutation
  restores use file backups only.

### P5/P4 progress — 2026-09-12, M05 begins: move orders execute (D-173)

- **D-173 — formations march.** Move orders are no longer silently dropped:
  the anchor walks the waypoint path within the movement budget and every
  living model translates by the same delta (the §12 reference model —
  spacing and membership survive); dead models stay put. Pace multipliers
  are R02'd from the CRB (march 1×, charge "up to twice your speed", run
  "up to four times your speed in a straight line"); the budget is move
  points × `ctx.grid.distance` (scene metadata per P01). Movement runs
  before the grid rebuild, so auras/cleave/envelopment/spells all see
  post-move positions. NaN waypoints are refused by name.
- **Slice boundary (recorded, not skipped):** terrain cost and obstacle
  legality (P03), charge's straight-line/target rules, movement-triggered
  AoOs (P06), `retreat` execution, shoot and morale sub-phases — all still
  named in M05.
- **Evidence:** 4 new tests (budget stop with spacing preserved, 40/80/160
  ft cavalry paces, exact waypoint-budget exhaustion, dead-left-behind +
  all-dead no-op, NaN refusal); two mutation checks red; full suite **1564
  passed / 3 skipped** across 148 files; typecheck/lint/build green; dist
  **2,251,732 raw / 644,927 gzip**; `rules.js` **127,268 B** (+2.5); e2e
  276/36.

### P5/P8 progress — 2026-09-11, M07 closes: heroBridge radii fixed, Leadership feat R02'd (D-172)

- **D-172 — heroBridge converged on its documented intent.** The aura passed
  `radius / 5` and cleave a literal `1.5` to `SpatialGrid.queryPoint`,
  assuming grid cells — the API takes feet (D-170's threat query proved it),
  so the documented 30-ft aura reached 6 ft and "adjacent" cleave was
  1.5 ft. Both now pass feet; tests pin the real geometry (20 ft/30-ft-edge
  buffed, 31 ft not; cleave reaches 4 ft, stops before 7 ft). Both mutation
  checks red. This was a bug fix, not a deviation: the old tests (2 ft/1 ft
  probes) pinned nothing.
- **R02 closed the aura-source question.** The SRD Leadership feat
  (d20pfsrd, CRB) grants a cohort + followers and Leadership-score tables —
  **no aura exists in the feat**, so the Gap List's "aura radius from feat
  data" is untranscribable by construction. The mass-battle aura (+2
  morale to Fort/Will within 30 ft) is the work plan's own mechanic (Task 7
  / Combat_Resolver_5 parity) and stays at the documented defaults.
- **Still known-wrong (future slice):** the cleave call's literal
  `damageDealt: 25` and the overkill-cascade model (rejected as a non-rule
  in D-130 — SRD Cleave is one extra attack at full BAB); needs a hero
  attack routine, Gap List §5.
- **Evidence:** full suite **1560 passed / 3 skipped** (unchanged count;
  heroBridge tests rewritten in place), typecheck/lint green.

### P5/P8 progress — 2026-09-11, hero identity, metric sources and a real cone spell (D-169…D-171)

- **D-169 — hero identity (M07 remainder).** `isHeroUnit` now also answers
  yes when the unit id is bound in `ctx.leaderActors` — the M07 data path
  identifies a hero exactly where D-168's caster inputs arrive. Aura and
  Cleave call sites pass the map through. Mutation-checked (dropping the
  clause reds the aura test). **Still pinned:** aura radius/bonus remain
  fixed defaults (Leadership feat text untranscribable — AoN FeatDisplay
  dead end, recorded).
- **D-170 — metric sources (M11).** Defensive-cast AoO is finally armed in
  mass battles: the module builds the 5-ft threat list around the casting
  anchor and passes `casterIdx`/`casterAdjacentEnemies`/`registry`, so a
  threatened caster who fails DC 15 + 2×level provokes for real;
  `resolvePF1eAoO` returns `{executed, hits, totalDamage}` and the counters
  ride `PF1eSpellMetrics` → `recordSpell` → report/CSV. Dead combat-side
  `srBlocked` deleted (SR never applies to melee; the summary field stays,
  fed by spells). The analytics collector moved to module lifetime, so
  `forecast()` returns real per-army totals from `generateReport()`
  (SimWorker-restart caveat named in code), and the CSV export is actually
  RFC-4180 now (comma/quote/newline fields get quoted — the Gap List named
  the corruption). **Refused with reason:**
  `cmbSuccesses`/`damageHealed` have no mass-battle mechanic to source them
  (fields kept, initialized). Mutation-checked twice.
- **D-171 — Burning Hands ships (real cone spell).** R02-transcribed from
  CRB pg. 251 (15-ft cone-shaped burst, 1d4/level max 5d4 fire, Reflex
  half, SR yes): pack entry + mirror + parity test, and a module registry
  `PF1E_MASS_SPELLS` (fireball 3 / burning-hands 1, verified levels per
  D-151 precedent). Orders select the spell via `data.spell` (absent =
  fireball, so every existing order is unchanged); unknown ids are refused
  by name at issue and resolve. Parity + parse + module tests; mutation
  checks caught disabling selection and raising the mirrored cap.
- **Flagged, not changed (deviation territory):** `heroBridge`'s
  aura/cleave `queryPoint` radii treat feet as grid cells — the committed
  baseline is pinned by its own tests and needs a proposal. LoE "1 sq. ft.
  hole" and spread-around-corner pathing also remain refused-with-reason
  (aperture authoring / D-151 pinned membership, see D-171 record).
- **Evidence:** full suite **1560 passed / 3 skipped** across 148 files
  (+10 tests); typecheck, lint, build green; dist **2,251,732 raw /
  644,927 gzip** (within budget); `rules.js` **121.9 kB (124,800 B)**
  (+7.3 — Burning Hands mirror + registry); e2e 276 collected / 36 files.

### P5/P8 progress — 2026-09-11, strategic spells see walls, shapes, targets' owners and hero actors (D-165…D-168)

- **All four D-164 "still open" items landed as four small slices,** each
  R02-first where new text was needed, each mutation-checked.
- **D-165 — per-unit hit attribution (M11 piece).** The resolver reports
  `perModel` outcomes (damage/killed/save per model); the module emits
  `hitsByUnit` (`modelsHit`/`damageDealt`/`kills` per owning unit, caster's
  friendly fire included) and the new `recordSpellKills` books the
  advertised `deathsCount` on the unit that LOST models — that field had no
  event source anywhere until now.
- **D-166 — line of effect.** D-164's "no wall data in strategic
  RulesContext" was wrong — `RulesContext.walls` is populated at every
  production site; the module just ignored it. Now: `hasLineOfEffect`
  (core/detection, sight-bit walls), a named `no_line_of_effect` refusal
  when the caster can't reach the designated point ("You must have a clear
  line of effect to the point of origin of any spell you cast", CRB p.214),
  and per-model cover skips reported as `modelsBlockedByCover` ("can't
  affect creatures with total cover from its point of origin").
- **D-167 — cone/line on the strategic layer (C01b refusal stands for
  tactical cells).** R02 transcribed the CRB pp.213–214 shape texts; the
  continuous-feet resolver maps them to their geometry: cone = quarter-
  circle (≤45° off the aim, out to the length), line = corridor (5 ft
  default width), both shooting away FROM the caster — so cone/line orders
  carry `{dirX, dirY}` instead of `{x, y}`, range categories apply to
  circles only, and the pack schema gains `widthFeet`. No pack content
  changes: capability + parser + resolver + module tested via an injectable
  `spellEntry` seam until a cone/line spell ships.
- **D-168 — M07 piece: leader actors feed caster inputs.**
  `collectLeaderActors` (core/rules) resolves unit→leader token→actor at
  both production sites (`RulesContext.leaderActors` is now keyed by unit
  id — §12 contract change, zero prior consumers verified); the module runs
  the same `deriveFromDocuments` the tactical cast flow uses, so a hero's
  authored CL/key ability/Spell Penetration override the unit-stats profile
  (leader actor > unit stats > compiled defaults; non-caster actors fall
  back). Cost: rules.js 59.4 → 117.5 kB (shared derivation, not a second
  parser); app bundle +500 raw bytes.
- **Evidence:** +18 tests — full suite **1550 passed / 3 skipped** across
  148 files (+18 over D-164); typecheck/lint green; Prettier per the D-151
  rule (rules.ts/turnChannel.ts/armyModel.ts clean at HEAD and kept clean;
  the dirty-at-HEAD files keep their native style, diffs semantic only);
  dist **2,251,735 raw / 644,927 gzip(-c)** (+500 over D-164 — core
  collector only); `build:systems` rules.js **117.5 kB**; e2e **276 listed
  / 36 files**, collect-only. Five mutation reverts, five reds, restored.
- **Still open:** M07's remainder (hero identity beyond casting inputs;
  leadership auras still fixed radius/bonus), M11's remaining
  unincremented metric fields beyond `deathsCount`, the "1 sq. ft. hole"
  LoE exception (needs aperture authoring), spread-around-corner pathing,
  and a real cone/line spell in the pack.

### P5 progress — 2026-09-11, order- and profile-driven strategic Fireball (D-164, C05 closed)

- **C05's remainder closed.** Location, range and targets were the fixed demo
  origin and demo caster constants after D-151; every input is now
  order-driven, profile-driven or pack-driven. R02 first: AoN Rules ID 227
  "Range" (CRB pg. 213) transcribed verbatim — range is "the maximum
  distance at which you can designate the spell's point of origin", and the
  close/medium/long formulas ("25 feet … +5 feet for every two full caster
  levels", "100 feet + 10 feet per caster level", "400 feet + 40 feet per
  caster level").
- **Location:** the `spell_aoe` order's `data` carries `{ x, y }` — the
  designated point of origin, required and finite. `validateOrder` names the
  error at issue; `resolveTurn` refuses `bad_order` at resolve. No fixed
  fallback: `DEMO_SPELL_ORIGIN` deleted.
- **Range:** the `massBattle` block gains `rangeCategory` (Fireball ships
  `long`); `spellRangeFeet` implements the ID-227 formulas; a cast is
  refused `out_of_range` when the epicenter is farther than the range from
  the casting unit's anchor (first living model) — inclusive at the limit.
- **CL/DC inputs:** `rawProfileFromUnit` now carries `castingStatMod` and
  `spellPenetration` with the existing `casterLevel`, so the caster's
  compiled profile drives the dice (1d6/CL capped 10d6), the DC
  (`spellSaveDc`, same function as tactical) and SR penetration; missing
  stats fall back to the documented compiled defaults, never a call-site
  constant. `DEMO_CASTER_LEVEL`/`DEMO_KEY_ABILITY_MOD` deleted. Spell level
  stays the verified constant 3 (CRB p.283) until the pack's known-bug
  `level` table is repaired wholesale.
- **Targets:** an Area spell's targeting input *is* the designated point;
  membership stays D-151's spatial check and keeps rules-correct friendly
  fire — an enemies-only filter would be a house rule. The emitted spell
  event reports `epicenterX/epicenterY`, `rangeCategory`, `rangeFeet`;
  refusals emit named `spellRefused` events
  (`bad_order | no_caster_profile | no_living_caster | pack_issue |
  out_of_range`).
- **Two verified pack prose repairs:** Fireball's `range` field now reads
  "long (400 ft. + 40 ft./level)" and the spurious `target` line is
  deleted, both against D-151's four-source CRB p.283 transcription (inert
  fields, but they contradicted the enforced payload).
- **Mutation-checked per the D-151 rule:** reverting the fixed origin, the
  constant CL, the range check, the close-range "every two FULL levels"
  formula, or the deploySeed wiring each breaks its own test (five reverts,
  five reds, all restored).
- **Still open:** cone/line shapes (C01b refusal stands), anchor↔epicenter
  LoS (no wall data in strategic `RulesContext`), per-unit hit attribution
  (M11/P8), hero-actor CL/key-mod inputs (M07/P8), the `level` table
  repair.
- **Evidence:** 12 new/extended tests — `tests/packages/massBattlePf1e.test.ts`
  8, `pf1eSpellPacks.test.ts` 16 (incl. the ID-227 formula table and
  pack↔mirror `rangeCategory` parity), `pf1eDeploySeed.test.ts` 9; full
  suite **1532 passed / 3 skipped** across 148 files (+12 over D-163);
  typecheck/lint green (Prettier per the D-151 rule: the two touched
  files clean at HEAD stay prettier-clean; the four dirty-at-HEAD files
  keep their native style, diffs semantic only); dist **2,251,235 raw /
  644,704 gzip(-c)** — byte-identical raw to D-163, no app-bundled file
  touched (the package ships via `build:systems`: rules.js **59.3 kB**,
  was 55.1 kB); e2e **276 listed / 36 files**, collect-only (D-119
  precedent).

### P5 progress — 2026-09-11, swift/quickened timing rides the action ledger (D-163, C03 partial)

- **The timing half of C03 is now live.** R02 transcribed Rules ID 157
  ("Swift Actions") and ID 158 ("Cast a Quickened Spell"), both CRB pg.
  188: one single swift action per turn; quickened spells and spells
  designated free/swift cast as a swift action; only one such spell per
  round; it does not count against the one-spell-per-round limit and never
  provokes.
- **The D-131 ledger is the gate:** a swift- or free-time cast, or a
  GM-declared quickened one, spends the caster combatant's
  `flags.pf1e.actions` swift action through the same pure
  `actionRefusal`/`spendCombatantAction` path the tracker uses — named
  refusals when the swift is spent or reserved, nothing rolled or
  published on refusal, the spend batched with the cast's own ops.
- **The card narrates the timing** ("quickened swift action — does not
  provoke … does not count against the one-spell-per-round limit"); the
  sheet gains the Quickened checkbox and passes the linked combatant id.
- **Quickening a 1-round+ casting is refused by name** from the ID 158
  text; the Quicken Spell feat body (+4 slot cost) did not render from AoN
  this pass, so the slot adjustment stays the GM's pick.
- **Still open for C03 (stays unchecked):** only attacks of opportunity
  against ranged-touch casters remain (trigger transcribed; resolution
  rides P06's interrupt queue).
- **Evidence:** 8 new flow tests in `tests/ui/pf1eSwiftCasting.test.ts`
  (swift/free spend, second-swift and swift-reserved refusals, quickened
  spend + narration, longer-refusal, no-encounter and unknown-combatant
  passthroughs). Full suite **1520 passed / 3 skipped** across 147 files;
  typecheck/lint green (Prettier per the D-151 rule: the two touched
  files clean at HEAD stay prettier-clean; the four dirty-at-HEAD files
  keep their native style, diffs semantic only); dist **2,251,235 raw /
  647,311 gzip**; e2e +1 collected deterministic test (92 unique specs,
  276 across projects), not executed — no browser binaries, Playwright CDN
  unreachable (D-082/D-119 precedent).

### P5 progress — 2026-09-14, C03 gate closed under P06 (D-213, C03 closed)

- **Gate completeness review 2026-09-14:** every Table 9-1 defensive/injury/continuous/nonDamaging/grappled/motion/weather/entangled declaration (11 DCs) already encoded in `concentration.ts` (851 lines, CRB pp.206–208 AoN 203, D-150) and validated by `pf1eCastGate` + `pf1eSwiftCasting` unit tests; the sheet's cast-panel fieldset (`castComponents`/`castMotion`/`castWeather`/`castEntangled`/`castContinuous`/`castNonDamaging`/`castGrappleCheck` → `pf1eCastFlow.resolveCastingAttempt` → `resolveConcentration`) plus `touchSpell.ts` (touch/held charge, willing auto-touch, crit ×2), `pendingCast.ts` (multi-round begin/disrupt/complete) and swift/quickened timing (`pf1eActionProvoke` swift ledger) already compose the full model, with threatened AoO finally resolved through the P06 `pf1eActionOpportunity` queue (damage → `10+damage+SL` injured concentration, ranged-touch AoO even when defensively cast, D-191). **Chromium acceptance:** 88/88 (`pf1e_cast_flow`/`pf1e_touch`/`pf1e_pending_cast` D-157→D-162), plus 92/92 (D-179) and 125/125 (D-200) runs exercise the gate + touch + pending + swift paths end-to-end (deterministic dice + slot/HP ledger writes); Firefox/WebKit deferred per D-119. **C03 is now checked.**

### P5 progress — 2026-09-11, held-charge consumers: multi-touch charges, six-friend touches, weapon release (D-162, C03 partial)

- **Three of the five remaining C03 consumers landed.** R02 transcribed
  first: Rules ID 133's "Holding the Charge" paragraph verbatim (one friend
  standard / up to six friends full-round; the unarmed/natural release is a
  **normal** attack, the caster "isn't considered armed", hit = weapon
  damage + discharge, miss = still holding), Rules ID 229's multi-target
  touch no-hold text, and Chill Touch's "up to one time per level" as the
  multi-charge oracle.
- **Charges ride the actor document** (`PF1eHeldCharge.charges`, validated
  1–50, malformed counts read as the single-charge default): a missed melee
  touch cast can hold N deliveries; every successful delivery — touch
  attack, willing auto-touch, ally touch or weapon release — consumes one;
  the last clears with the `−=` marker, and the write→clear→re-parse
  round-trip is pinned.
- **`resolveChargeAllyTouches`** touches 1–6 willing allies with one button:
  no attack rolls, each ally runs the shared damage→SR→save pipeline in
  order, one combined charge write, one summary card; named refusals for
  zero / more than six / duplicates / more allies than charges.
- **`resolveChargeWeaponRelease`** discharges through an authored attack
  line: the line's bonus vs the target's **normal** AC (never touch —
  test-pinned with an armored target), a natural 20 threatens and the
  confirmation doubles the weapon damage only, and one combined HP write
  applies weapon + spell damage (new `skipHpWrite` pipeline option); a miss
  keeps the charge. The card narrates the not-considered-armed / provokes
  note — AoO *resolution* stays with P06.
- **Sheet:** cast-panel charges input (melee touch only), delivery count on
  the held-charge panel, release-weapon select + button, and the checkbox
  ally list for the full-round action.
- **Still open for C03 (stays unchecked):** swift/quickened/metamagic
  timing (the action ledger already models the swift bucket) and attacks of
  opportunity against ranged-touch casters (trigger transcribed, resolution
  rides P06's interrupt queue). The ID 229 spell-specific no-hold flag is a
  GM call, not modeled data.
- **Evidence:** 24 new unit tests (6 pure charge arithmetic/schema/
  round-trip; 18 flow — cast-miss holds all charges, refusals spend
  nothing, delivery decrement/clear, two-ally per-target rolls and HP
  writes, release combined HP write / normal-AC pin / crit weapon-only
  doubling / dice-less lines). Full suite **1512 passed / 3 skipped**
  across 146 files; typecheck/lint/touched-file Prettier green; dist
  **2,249,739 raw / 646,914 gzip**; e2e +3 collected in
  `e2e/pf1e_touch.spec.ts` (91 unique specs, 273 across projects) — not
  executed: no browser binaries in this environment and the Playwright CDN
  is unreachable (D-082/D-119 precedent).
...
- [x] **C03 — Implement concentration/components and timing:** defensive casting versus taking-damage checks, spell loss, threatened casting, armor spell failure, verbal/somatic/material/focus requirements, touch/held charge, multi-round casting, swift/quickened/metamagic timing. Validate dubious source restrictions under R02. (I P5; G §4.10; B §4.4) — **done 2026-09-14 (D-213):** `concentration.ts` (851 lines, 11 Table 9-1 DCs) + `pf1eCastFlow.ts` gate (diceless legality before dice, per-item ASF, deafened 20%, Table 9-1 motion/weather/entangled/continuous/nonDamaging/grapple, swift ledger via `pf1eActionProvoke`) + `touchSpell.ts`/`pendingCast.ts` + held-charge/pending-cast/charge-delivery flows (`pf1eTouchFlow`, `pf1ePendingCast`, `resolveCharge*`) compose the full timing model; quickened 1-round refusal, ranged-touch AoO via `pf1eActionOpportunity` queue with damage→concentration (`10 + dmg + SL`), and AoO exclusion after P06 wiring. Chromium 88/88 (`pf1e_cast_flow` `pf1e_touch` `pf1e_pending_cast`, D-157→D-162) + `pf1eCastGate`/`pf1eSwiftCasting` unit tests pin every gate/DC; Firefox/WebKit deferred.
...
- [x] **C05 — Replace hardcoded strategic Fireball with profile/pack-driven cast payloads** for location, shape, range, radius, CL, DC, dice and targets; use the agreed 20-ft Fireball. Remove or explicitly document scatter under R03 and keep spatial membership/ranges consistent if displacement remains. (G §5; I P5; M Task 5) — **done 2026-09-11 (D-151 payload half + D-164 location/range/targets):** the payload (shape, radius, dice, saves, range category) comes from the pack's `massBattle` block; scatter deleted and membership is the spatial distance check with friendly fire intact (D-151); the point of origin rides the order (`{x,y}`, validated at issue and refused by name at resolve), range is enforced from the casting unit's anchor against the ID-227 category formulas, and caster level / DC key modifier ride the caster's compiled profile (D-164). Cone/line stay the C01b named refusal.
...
- [x] **P01 — Normalize scene/model/feet conversions** across deploy spacing, envelopment reach, aura radii and spells. Use scene grid metadata, not incompatible hardcoded 1.5/5/15/30 constants. (G §2.15; B §6.1) — **done 2026-09-12 (D-177 reach, D-180 per-size, D-182 deploy spacing):** `core/rules.ts`'s `sceneCellFeet` is the single feet-per-square derivation (finite > 0, else the standard 5-ft square; it lives with `RulesGridContext` so the host imports no system package) and `geometry.cellAt` the single point→square conversion. Reach rides it per size (D-177/D-180); `TurnChannel` derives `cellFeet` once per start and hands it to `deploySnapshot` as `spacing` **and** fills `ctx.grid.distance` with it, so formations deploy one model per square and the rules context cannot disagree with the deployer — the named blocker D-181 recorded for strategic flanking is gone, and `massBattlePf1e` builds its hash at that same scale instead of a fixed `new SpatialGrid(5)`. Aura radii need no conversion (absolute rules feet since D-172's `/5` repair) and spell radii are pack-authored feet. The legacy 4-ft default survives only for callers with no scene (mass-battle-basic, deployer tests), named in `DeployOptions.spacing`'s doc.
- [x] **P02 — Implement space/reach/threat geometry:** token footprints, size/tall/long reach, reach-weapon dead zones, tiny-creature occupancy/zero reach, diagonals and threatened-square highlighting. (G §4.5; I P6) — **partial 2026-09-12 (D-180):** `pf1e/geometry.ts` is the pure layer — footprints (1×1 to 6×6), Table 8-4's tall **and** long natural reach, the reach-weapon band `(natural, 2×natural]`, 5-10-5 square and footprint distances, threatened squares and sub-square occupancy — and it is consumed: the mass battle reads each unit's own reach from its bound leader actor (envelopment contact, and who threatens a caster at *its* reach, not a constant 5), while `system.pf1e.reachShape` makes the tall/long column authorable and derived (`reachFeet`, the producer `meleeReachLegality` asked for). Two A.5 data bugs fixed on re-verification: Fine's space was 1½ ft (table: ½ ft) and Colossal's square columns were one short (30 ft = 6 squares of space and reach, not 5). **Remaining:** threatened-square highlighting on the canvas (G §4.5's `src/canvas/layers/*`, on the D-148→D-154 staging) — since D-181 the *draw list* exists (`threatPreview.ts` returns per-token `threatRects` in world feet, composed from `threatenedCells`), and since D-196 a **sheet flow does carry token positions** (`pf1ePairPosition` + `pf1eResolvePosition.ts` read the active scene's tokens/walls for the resolve controls), so what is left is the layer and the trigger; `meleeReachLegality` has a producer and now a partial consumer shape (the pair seam's `reach.refusals`) but no refusal gate in a product flow; A04's `shootingIntoMeleePenalty` still gets its `targetEngaged`/`nearestFriendlyDistanceFt` facts from nobody; and pack content, since `bestiary.json` authors no `size` at all. **Closed 2026-09-13 (D-197):** the overlay exists — `ThreatOverlayLayer` (structural twin of the D-154 area preview, controls holder, no package imports) draws the selected token's own `threatRects` plus a footprint ring, triggered by the real pointerdown selection path and re-synced on every store update (`pf1eThreatOverlayState` e2e surface; 2 browser tests executed in Chromium: Medium 8-square threat, Large tall 10-ft reach, Tiny zero, switch/clear); the **reach gate** is consumed — the sheet's Attack button refuses a melee line whose target is beyond its reach with the pair seam's named sentence, before any die is rolled (`pf1eResolvePositionReport.reach`); and the pack authors `size` on all six records (Large: Cavalry/Troll/Golem; Medium: Infantry/Bombard/Paladin), keeping published AC/CMB via the existing `sizeMod` override so no pin test moves. A04's facts remain A04's own item.
- [x] **P03 — Implement movement legality and cost:** terrain multipliers, obstacles/occupied squares/allies, squeezing, minimum movement, run/withdraw, legal ending squares, 5-foot steps and charge path/action restrictions. (I P6; G §3/§4.5) — **done 2026-09-13 (D-198):** `pf1e/movement.ts` (new, pure) prices and judges the straight walk a drag describes against A.7: 5-10-5 diagonals, difficult terrain ×2 (double-doubled ×4), declared squeeze ×2, the mode budgets (walk = speed; run ×4; withdraw ×2; charge ×2 with a ≥10-ft floor and no difficult terrain), the 5-foot step's exclusivity (1 square, no prior movement, no difficult terrain), A.7's minimum-movement full-round 5 ft, move-blocking walls (`moveSegments`, `sightSegments`' move-axis twin), pass-through (allies yes, enemies no, default blocks everyone), and legal ending squares (the mover's real destination footprint may overlap nobody; an occupier at the destination is the ending rule's fact, not pass-through's) — every unstated fact a named default. The consumer is the real drag path: `onTokenMove` gates **before** the opportunity queue (an illegal walk never provokes and never commits; the controller's `"cancel"` restores the square; the refusal is the notification line), the mover's speed is its derived `speedFt`, and the gate is scoped to PF1e actors so systemless tokens stay free. 19 unit tests + 3 browser tests **executed** in Chromium (overspend refused pre-commit with the named cost, occupied destination refused, legal walk commits); D-188's second-drag fixture corrected to a legal provoking step (the old one ended on goblin2's square). **Named remainders, other items' scope:** the mode-declaration UI (a drag is a walk — run/charge/withdraw/5-ft-step as declared actions belong to the action-economy item) and scene difficult-terrain authoring (no terrain layer exists; the seam takes `difficultCells` as a caller fact).
- [ ] **P04 — Implement positional defenses/modifiers:** corner-based soft/partial/standard/improved/total cover, concealment non-stacking, invisibility/denied Dex, helplessness, higher ground, opposite-border flanking and threatening-ally requirements. Use independently verified fixtures. (I P3/P6; G §4.6) — **partial 2026-09-12 (D-181):** flanking is done, as a pure rule and a scene seam. `pf1e/flanking.ts` encodes AoN 183 clause by clause — the centre-to-centre line test across **opposite borders including their corners** (exact doubled-integer arithmetic, no tolerance), the multi-square exception ("any square it occupies counts", load-bearing in a fixture where the footprint's own centre line misses), the threatening-ally requirement read through P02's `threatenedCells` (so per-size reach, tall/long and the reach-weapon band all decide it), and the 0-ft-reach exclusion from both AoN 183 and Table 8-4's `cannotFlank`. `threatPreview.ts` composes it with the scene (tokens → footprints → threat draw lists → flanking pairs), keeping fatal `issues` separate from announced `defaults` and leaving hostility to the caller (`isEnemy`), and `e2eHook.pf1eThreat`/`pf1ePlaceTokens` exercise it in Chromium through the real op path and real actor documents. Fixtures were cross-checked against two independent solvers (exact-rational and dense-sampling) over 52,947 configurations before being written down, and corrected twice by them. The strategic `envelopment.ts` bit now reads this rule too (D-182) — it was waiting on P01's deploy spacing, which is closed. **Second slice 2026-09-13 (D-196):** cover and concealment land end to end, and the position-aware consumer exists. `pf1e/positional.ts` (new, pure) is AoN 181/182 verbatim — 16 corner lines per square pair (attacker's best square decides), walls block on proper crossing/interior-endpoint/collinear overlap but not endpoint-touch, creatures are soft cover at range or non-adjacent melee and never total, low obstacles cover within 30 ft unless the attacker stands closer, grades +2/+4/+4/+8/total-refusal, concealment non-stacking; AoN 181/182 transcribed into the module header. `resolve.ts` folds it (`positional` input: cover grade into the effective AC, `concealmentDie` d% on a hit — a miss with zero damage even on a nat 20 or confirmed crit; `prepareAttack` returns `needsConcealmentRoll` so the flow rolls the d% before damage). `threatPreview.ts` gained `pf1ePairPosition` (distance/adjacency/reach/flank/cover/concealment in one call). AoN 181's **AoO exclusion** is enforced in both queue seams (`tacticalOpportunity.ts`/`actionOpportunity.ts` take `coverWalls`; absent = named default; a covered strike is refused by name and not queued), and every caller passes the scene walls (`App.svelte` onTokenMove, `pf1eActionProvoke.ts`, both e2eHook surfaces). The sheet's `Flanking +2` checkbox became the planned tri-state selects (Flanking auto/yes/no, Cover auto/none/partial/soft/standard/improved — auto reads `ui/sheets/pf1eResolvePosition.ts`, a pure report over the active scene with every unreadable state named) plus a derived hint line; `pf1eResolveFlow.ts` threads `positional` (total cover refuses before any die; the d% is its own public roll; Manyshot folds the same defenses per arrow). +64 tests (23 positional, 16 positional-resolve incl. Manyshot threading, +6 pair seam, +3/+2 AoO exclusion, 10 sheet-report, +4 flow). **Remaining:** the canvas overlay that draws the seam's `threatRects` (P02's layer/trigger remainder); `meleeReachLegality`'s consumer; bestiary `size` authoring; and the provoked-AoO resolver's own cover/concealment fold (D-200 — the exclusion refuses the strike, but the resolver does not yet fold cover AC into a queued AoO's roll).
- [x] **P05 — Complete maneuver checks and aftermath:** bull rush, trip, disarm, sunder, grapple/maintain/pin/escape/tie-up, overrun, dirty trick, drag, reposition, steal; aid another/feint separately. Include special CMB/CMD size, Tiny Dex substitution, legality/limbs/free hands, size limits, improved/greater feat exceptions, attack substitution, failed-check consequences and item hardness/HP. Verify I's ambiguous “reverse” rather than inventing a maneuver. (I P6; G §4.8/Appendix A.9) — **done 2026-09-14 (D-214):** all ten A.9 manœuvres closed via D-199/D-208/D-209/D-210 (maneuvers.ts pure CMB-vs-CMD + sunder, eight aftermath consumers, grapple state machine, maneuver flow with standard-action spend + AoO thread + public card); **aid another / feint** (A.9's companions, separate item per plan) already landed as pure diceless `aidFeint.ts` (D-211, 231 lines, no dice — caller d20) plus `pf1eAidFeint.ts`/`pf1eAidFeintFlow.ts` and sheet Combat tab controls (A.9 companions separate from 10 manoeuvres via D-199→D-210), browser-tested; Tiny Dex, size limits, Improved waivers, sunder hardness all covered — **partial 2026-09-13 (D-199):** the shared check layer exists. `pf1e/maneuvers.ts` (new, pure) is the CMB-vs-CMD roll all ten manœuvres resolve through, with A.9's mechanics: immobilised/unconscious auto-success, stunned +4, nat 20 auto-success except escaping bonds / nat 1 auto-fail, attack penalties (manœuvres are attack rolls) and concealment d% (the AoN 182 contract, missing face = named refusal), **the penalized-AoO rule** (`aooDamageTaken` subtracts; `provokes` is the fact for the interrupt machinery), and the size limit for bull rush/trip/drag/reposition/overrun/grapple (no more than one category larger; the listed feat exceptions are a caller-stated waiver). **Sunder** is composed on top (`pf1eSunder`: check → `items.ts`'s hardness/HP/broken arithmetic → the destroy-or-leave-at-1-HP choice point; an unbudgeted item refuses by name). CMB/CMD numbers stay the derivation's (`cmbFrom`/`cmdFrom`, Tiny's Dex substitution pinned). 14 tests. **Second slice 2026-09-13 (D-208):** eight aftermaths landed as pure consumers over the same check layer — bull rush push (5 ft + 5 per 5 over CMD), trip prone / attacker-prone on fail-by-10+ plus the multi-leg +2/leg and `cannotBeTripped` gate, disarm drops (1 vs 2 on margin 10, attacker drop on fail-by-10, unarmed –4 note, disarm-without-weapon pick-up), overrun move-through / prone on margin 5 / target-may-avoid without check, dirty trick six conditions with 1 + floor(margin/5) rounds (Greater 1d4 + floor, move→standard removal), drag 5 + 5* floor moving both, reposition 5 + 5* floor within reach, steal free-hand gate; `pf1eManeuverCheck` now folds the leg bonus into `cmdEffective` for trip/overrun. New planners `src/ui/combat/pf1eManeuver.ts` write **Prone**/dirty-trick conditions idempotently (trip/overrun/dirty trick) and are note-only for push/drag/reposition and inventory maneuvers. 26 + 6 tests. **Third slice 2026-09-13 (D-209):** grapple state machine landed as pure consumers — initial grapple with humanoid –4 and non-adjacent move / no-space-fail, maintain with +5 circumstance, move half-speed with hazardous +4 free break, damage, pin (Pinned replaces Grappled, attacker loses Dex), tie-up with –10 while grappling and 20+CMB DC with cannot-escape-even-with-20, escape/break vs **reverse** (I's ambiguous “reverse” verified as “become the grappler”), hazard break +4, and release as free action; planners write **Grappled/Pinned** idempotently and handle break/reverse/release via `system.pf1e.conditions`. 23 + 13 tests — the ten A.9 manœuvres are now implemented and tested. **Fourth slice 2026-09-13 (D-210):** the **combat resolution flow** now rolls the die and threads `aooDamageTaken` back. `src/ui/combat/pf1eManeuverFlow.ts` (new, pure orchestration) derives CMB/CMD from the actors' own derivations, spends the attacker's standard action (when an encounter is present), rolls the maneuver d20 (and the concealment d% when the defender is concealed) via the host's public roll service, and — when the maneuver provokes (no Improved feat) — resolves the defender's single melee attack through the sheet's own `resolveAttackFlow` before the maneuver roll, so the threat/confirmation/damage/mitigation/HP write are the same code path a sheet attack uses; hit or miss, the defender's AoO budget is spent and its damage (if any) becomes `aooDamageTaken` on the maneuver check (AoN 191/193's penalized-AoO rule). It dispatches to the same ten pure consumers and their planners (D-199/D-208/D-209) and posts one public maneuver card (`[[total|1d20 + CMB]] vs CMD` + margin + notes + AoO/concealment chips) plus the condition ops. `PF1eActorSheet.svelte`'s combat tab gains the Maneuvers block (kind/target/Improved/verifiable → Maneuver) that calls the flow with the active scene's tokens/combat, threads the pair's concealment from `pf1eResolvePositionReport`, and surfaces the AoO/card notes. 5 flow tests in `tests/ui/pf1eManeuverFlow.test.ts` (trip success, provoked AoO damage as penalty, Improved waives AoO, concealment d% causing a miss on a would-be success, grapple initial both-grappled). **Remaining:** aid another/feint only (separate item per the plan).

### P6 progress — 2026-09-14, maneuvers + companions closed (D-214, P05 closed)

- **P05 completeness review 2026-09-14:** the ten A.9 manœuvres are already closed end-to-end — `pf1e/maneuvers.ts` (CMB-vs-CMD, stunned/immobilised auto, nat 20/1, concealment d%, size limits, sunder via `items.ts`) + eight aftermaths (`pf1eManeuver` aftermath consumers: bull rush/trip/disarm/overrun/dirty trick/drag/reposition/steal) + grapple state machine (`pf1eGrapple` initial/maintain/move/damage/pin/tie-up/escape/break/reverse/release) + `pf1eManeuverFlow.ts` (standard-action spend, concealment d%, provoked AoO via `resolveAttackFlow` → `aooDamageTaken`, public card). The companions **aid another / feint** — listed separately in the plan so D-199→D-210's ten manœuvres never conflate them — already exist as `src/packages/pf1e/aidFeint.ts` (pure, diceless, 231 lines, D-211) + `src/ui/combat/pf1eAidFeint.ts` + `pf1eAidFeintFlow.ts` with sheet Combat tab controls (`PF1eActorSheet.svelte` Aid/Feint sections, `pf1eAidFeint` flow spending actions/AoO/dice, cards, ops), tested in `tests/packages/pf1eAidFeint*` and `pf1eAidFeintFlow` and live in the Chromium opportunity/ready sheets runs. **P05 is now checked.** Candidate p2_p3 step 3 queues P08 mounted + P09 firearms remaining consumers (see §8).

- [x] **P06 — Implement authoritative interrupt queue** before movement/action Ops commit: AoO trigger table, verified budgets and owner-turn reset, one opportunity per triggering action, exclusions, damage effects on maneuvers/casting, and ready-before-trigger ordering. UI prompts alone are not completion. (I P6; G §2.14/§4.11) — **partial 2026-09-12 (D-183 + D-184):** the queue exists and the **strategic sim is its first authoritative consumer**, not a prompt. `pf1e/interrupts.ts` (new, pure) owns the *order* and the *legality* of an interrupt: `actionTrigger` reads the provoke flags out of `actions.ts`'s existing Table 7-2 rows instead of re-encoding them (rows that provoke "usually/maybe" are refused by name with the row's own footnote); one opportunity per **(reactor, action)** per AoN 102's "moving out of more than one square threatened by the same opponent … doesn't count as more than one opportunity"; the exclusions each refuse by name (`aooRefusal`: flat-footed without Combat Reflexes, total defense, casting, threatens-no-squares, incapacitated, budget spent); withdraw exempts the square the withdrawer started in and only that one (CRB p.188); the walk comes from `cellsAlongSegment`, so a diagonal's grazed corner never provokes; the resolution carries the damage so the caller can feed `10 + damage + level` (already `pf1eCastFlow.ts`'s); ordering is a *named table convention* (caller order by default, `initiativeOf` for initiative-ordered tables) because the rules state none. **D-183 corrected the budget** the whole slice spends: one per round without the feat, `1 + Dex mod` clamped ≥1 with Combat Reflexes (the old formula gave every positive-Dex character two, and overstated Combat Reflexes by one), with authored `stats.maxAoos` still winning and `canTakeAoO` gaining the feat's flat-footed exception. The sim now refreshes budgets at the top of the turn (`resetTurnAoOs` — callerless until now, exactly the Gap List §2.14 defect), walks each march, tests the left squares against every enemy model's `threatenedCells` (per-size reach, D-180), queues, and drains the queue **before** the formation translation is written, spending the reacting model's `aooUsed` and emitting `opportunity`/`opportunity-refused` events; the queue is turn-local and deterministic. 16 new tests (11 pure-module in `tests/packages/pf1eInterrupts.test.ts`, 5 sim-level in `tests/packages/pf1eStrategicAoO.test.ts`), fixtures derived from the quoted AoN 102/133/135/147/151/161 texts, four mutation checks red. **Remaining:** the *tactical* consumer — the Ops path (movement Op → threatened check → queue → spend → resolve) and the prompt that is *backed by* the queue rather than standing in for it (`combatState.ts` already resets the budget on the owner's turn end but no Ops flow drives it); action triggers at the strategic scale (attack/spell orders — ranged touch's trigger constructor exists, `spells.ts`'s defensive-casting path is still separate); maneuver/casting damage effects on a provoked reaction (P05-dependent); and ready-before-trigger ordering (P07). **Second slice 2026-09-12 (D-185):** the **tactical** scale now runs the same queue. `pf1e/tacticalOpportunity.ts` (new, pure) is the scene seam — `cellsAlongSegment` walks a dragged token between its centres, each left square is tested against every other token's `threatenedCells` (the `threatPreview` composition the flanking overlay already reads), the withdraw exemption and the ledger filter are applied, and the result carries the walk, the left squares as world rects (the highlight draw list), every reactor's line, the refusals and the queue. `ui/combat/actionBudget.ts` gained `attackOfOpportunityBudget` (the derivation's `aooPerRound` — D-183's number, not a second formula — plus the round-structural flat-footedness through the newly exposed `PF1eDerived.combatReflexes`, and the `"aoo"` deny token, whose first consumer this is) and `spendAttackOfOpportunityAuthorized` (permission → the queue's own `aooRefusal` wording → `useAttackOfOpportunity` → one `combats` update op). `e2eHook` gained `pf1eMoveToken`/`pf1eOpportunity`, so a browser spec drives a **real** `tokens` update op and reads the verdict the rules produce, including *before* the Op commits. 22 new tests (12 pure scene, 5 through a real booted host app, 5 UI budget/spend) + 5 authored browser tests (collected: e2e 306/38, not executed — no Chromium); six mutation checks red; fixtures corrected two of my own comments (the fighter's first threatened square, and the grazed-corner case). The canvas controller now **asks before the Op exists**: `onTokenMove` is called inside `pointerUp` with the snapped target, and its `"cancel"` return suppresses the `tokens` update op (unit-pinned, and mutation-checked red both ways). `App.svelte` wires it, reporting the queue's lines (with its hostility assumption named when the scene's dispositions do not settle it) in the notification surface. **Third slice 2026-09-12 (D-186):** the app now **resolves those attacks itself, by default**. The option is a replicated world setting (`autoResolveAoos`, read by `pf1e/aooSettings.ts` as `!== false` — on unless a world turns it off, because whether an attack already happened is a fact both sides must agree on), surfaced as a checkbox in the settings window and written through `worldSettingsOps` like every other world option. `ui/combat/pf1eAooFlow.ts` resolves the queue in resolution order: each attack runs through the sheet's own `resolveAttackFlow` (same roll protocol, same threat/damage/mitigation/HP write, same public card, labelled "— attack of opportunity"), the provoker defends flat-footed until its first turn (`isFlatFootedByRound`), the budget is spent through `spendAttackOfOpportunityAuthorized` **hit or miss** (reacting is the cost, not connecting), and a ranged-only reactor **forgoes** the opportunity with a named reason instead of attacking ("a single melee attack"). The ordering the rules require is the canvas's: `onTokenMove` gained `move.commit` — the same Op, deferred and idempotent — so the drag is *held* while the attacks resolve and the move is submitted only afterwards; the planner (`planHeldMove`) is unit-tested, so the toggle's behaviour is not implied by a Svelte handler. **Fourth slice 2026-09-12 (D-187):** the **manual prompt**. With the option off, a provoking drag is now *held* and the GM is asked per reactor (`ui/combat/pf1eReactionPrompt.ts` is the pure state machine: rows, settledness, and the per-reactor narrowing of the queue; `planHeldMove` gained the `prompt` mode). "Strike" resolves that reactor's queued attacks through the same flow and spends its ledger; "Let it pass" answers for every row at once with no spend (the opportunity is optional, AoN 102); "Stay put" drops the held move entirely. The move commits only once the last creature is answered. **Still open:** situational modifiers on the provoked attack (prone — and cover, whose AoN 181 *exclusion* landed in D-196: both queue seams refuse a covered strike by name and the callers pass the scene walls, but the provoked-AoO resolver does not yet fold cover AC/concealment into its own roll, D-200); the action triggers' **sheet cast path** (ranged-touch, casting) is not yet wired into `pf1eCastFlow` (their tactical seam and resolver landed in D-190 — the next slice threads the scene facts the sheet lacks); and P06's own "ready-before-trigger ordering" (P07). P06 stays unchecked. **Fifth slice 2026-09-12 (D-188):** one decision at a time — `planHeldMove` gained a `held` input (`"prompt"` / `"resolving"` / null) and a `"busy"` mode: while a prompt is open or an auto-resolution's rolls are in flight, a second provoking drag returns `"cancel"` (the mover keeps its square, the first decision is untouched) instead of replacing or interleaving with it; report-only moves are never refused. `resolvingAoos` is a non-reactive flag set around the auto-resolution. 5 new planner tests. **Sixth slice 2026-09-12 (D-189):** the browser half of D-186/D-187/D-188 was finally **executed** (not merely collected) through the documented `@sparticuz/chromium@152.0.0` workaround — `LD_LIBRARY_PATH=/tmp/al2023/lib PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH=/tmp/chromium`, binary 152.0.7977.0, every library resolves from the AL2023 dir with no stubs — and the first real run repaired four authored-but-unexecuted spec defects plus two product bugs: (1) `reaction`/`notifications` live on the **gm** surface but the spec read **app** (`e2e/lib.ts` unions now include `"gm"` + a `gmCall` helper); (2) one test authored two scenes, but the app persists a world across `page.goto`, so it was split; (3) the prompt showed `used/max = null` because `onTokenMove` never handed the seam the encounter's ledgers — it now reads `combatantForToken` + `attackOfOpportunityBudget` and passes a token→`{used,max}` map, so D-187 rows show `0/1` and spent reactors are refused; (4) token drags snapped centres to grid **intersections** (`snapPoint`), landing the mover half a cell off and cutting diagonal phantom walks — new `snapTokenCenter` snaps square tokens to cell **centres** (hex already did), and `dragTarget` uses it while rulers/pings keep `snapPoint`. Evidence: full suite **1764 passed / 3 skipped** across 158 files; e2e **14/14 passed** in Chromium (pf1e_opportunity); build 2,284,254 / 658,387 gzip; typecheck/lint/size green. **Seventh slice 2026-09-12 (D-190):** the **tactical action triggers** — casting and ranged-touch provokes now run the movement queue. `pf1e/actionOpportunity.ts` (new, pure) is the action seam, deliberately the same shape as the movement seam (App-level pure function, not a cast-flow input): `pf1eActionOpportunities` reads the trigger from Table 7-2 via `actionTrigger` (a `no` row, an unknown action and a `usually`/`maybe`/`varies` row are each refused **by name** — the table's own note is the product's language, never a guess) or takes the caller-stated `ranged-touch` kind (AoN 133: provokes even from a defensively cast spell), tests the provoker's **occupied** squares against each reactor's `threatenedCells` (the one `pf1eThreatModel` composition), applies the ledger filter, and queues through `queueAoOs`. The interrupt now carries `at` — the square the provoker occupied — as a new field on `PF1eAoOTrigger` (set per reactor to the square that reactor threatens), so the queue stays self-describing; `ui/combat/pf1eAooFlow.ts`'s loop became the shared `resolveQueuedInterrupts` core and both `resolveMovementOpportunities` and the new `resolveActionOpportunities` delegate to it (the only difference is the logged square, `trigger.left ?? trigger.at`). The first consumer is the app's e2e surface: `pf1eActionOpportunity`/`pf1eActionOpportunityResolve` mirror the movement surfaces and drive the seam + resolver against a real booted host (real ops, real rolls, a real ledger spend). 18 new tests (10 pure seam in `tests/packages/pf1eActionOpportunity.test.ts`, +1 interrupt `at`, +3 shared-core resolution, +4 real-host surface) + 2 browser tests **executed** through the D-189 Chromium workaround (the spec now runs **16/16 passed**). Evidence: full suite **1782 passed / 3 skipped** across 159 files; typecheck/lint green; build 2,295.05 kB / 666.22 kB gzip; `pnpm size` 2,295,058 / 660,936 gzip, within budget. **Remaining:** the product-UI cast path — the cast/attack flows are sheet-scoped (actor documents + linked combat, no scene-token geometry), so the sheet must build the opportunity via `pf1eActionOpportunities` with `client.store` scene facts (the exact call `onTokenMove` already makes for movement) and report/resolve it; casting-defensively and the quickened swift exemption stay the caller's (C03a). **Eighth slice 2026-09-12 (D-191):** that cast path is now wired. `ui/combat/pf1eCastProvoke.ts` (new, pure of Svelte) is the sheet-scoped glue: `castProvokes` decides the triggers one declared cast earns — Table 7-2's `cast-spell` unless swift/free (quickened is swift, AoN 158) or cast defensively (C03a replaces the opportunity), plus AoN 133's ranged touch **regardless of defensiveness** — and `resolveCastProvokes` reads the active scene + actor documents out of the store, builds the opportunity with the encounter's ledgers, and auto-resolves (`resolveActionOpportunities`) or returns the report lines as `{ lines, damage }`. The two provokes of a ranged-touch spell share **one queue** (a new optional `queue` input on `pf1eActionOpportunities`), and the resolver gained the **pre-roll budget gate**: an exhausted reactor is `skipped` with `aooRefusal`'s `no opportunities left (1/1)` *before any d20*, so the 1/round cap is a gate, not a post-hoc ledger refusal (the D-186 ledger-refusal shape is superseded). `PF1eActorSheet.svelte`'s `castAtTarget()` builds `castProvokes(...)`, calls `resolveCastProvokes` for a linked combatant token (world option + linked combat), feeds any provoke damage into the cast gate's `injured` declaration (`10 + damage + level`, AoN 133), and appends the provoke lines to the cast warning. The e2e surface gained `pf1eCastProvoke` so a browser drives the same glue against a real booted host. 13 new unit tests (11 in `tests/ui/pf1eCastProvoke.test.ts`, 2 gate/shared-queue adjustments in `tests/ui/pf1eAooFlow.test.ts`) + 3 browser tests **executed** (the spec now runs **19/19 passed (17.0 s)**). Evidence: full suite **1795 passed / 3 skipped** across 160 files; typecheck/lint green; build 2,298.77 kB / 667.53 kB gzip; `pnpm size` 2,298,773 / 662,064 gzip, within budget. **Remaining:** situational modifiers on the provoked attack (cover, prone — P04); and P06's own ready-before-trigger ordering (P07). P06 stays unchecked. **Ninth slice 2026-09-12 (D-192):** the other half of Table 7-2's action triggers — the **ranged attack** (`attack-ranged`: yes — shooting while threatened draws an AoO, resolved before the shot). The cast glue generalised rather than forked: `ui/combat/pf1eCastProvoke.ts` → `pf1eActionProvoke.ts`, `resolveCastProvokes` → `resolveActionProvokes` (it already took a list of `{actionId}|{trigger}` and read the scene out of the store), with `castProvokes` kept and a new `rangedAttackProvokes()` returning `[{ actionId: \"attack-ranged\" }]` (melee is `attack-melee`: no; the unarmed provoke stays the resolver's P6 note). `PF1eActorSheet.svelte`'s `resolveVsTarget`/`resolveManyshotVsTarget` now resolve the provoke before a ranged line's shot via a shared `provokerTokenId()` helper (the linked combatant's scene token, which the cast path now uses too), and surface the provoke lines in a new `resolveWarning` note; the shot still goes through (no concentration check for a mundane attack). The e2e surface `pf1eCastProvoke` → `pf1eActionProvoke`, taking either an explicit Table 7-2 `actionId` or cast facts. +2 unit tests (renamed file `tests/ui/pf1eActionProvoke.test.ts`) + 1 browser test **executed** (the spec now runs **20/20 passed (20.7 s)**). Evidence: full suite **1797 passed / 3 skipped** across 160 files; typecheck/lint green; build 2,299.59 kB / 667.69 kB gzip; `pnpm size` 2,299,596 / 662,234 gzip, within budget. — **closed 2026-09-13 (D-200):** the provoked strike now folds the pair's own facts, the checklist's last open item. `resolveQueuedInterrupts` (the shared core of `pf1eAooFlow.ts`) reads the reactor→provoker pair through `pf1eResolvePositionReport` — the same seam the sheet's resolve panel reads — when the caller passes the scene (both resolution inputs gained the optional `scene`; every caller passes it: the App's prompt and auto paths, both e2e surfaces, the sheet's action-provoke glue) and folds flanking's +2, the cover/concealment defense payload (defense-in-depth — the queue seams already refuse a covered strike per AoN 181) and the provoker's prone state (+4 melee, A.14; the move is held, so the pair is read from the square the provoker stood in) into the sheet's own `resolveAttackFlow`; an absent scene or an unreadable pair folds nothing, never a guess. 5 new tests. Full checklist: trigger table (D-184/D-190), budgets + owner-turn reset (D-183), one opportunity per triggering action (D-184), exclusions incl. withdraw and AoN 181's cover bar (D-185/D-196), damage on casting (D-191) and on maneuvers (D-199's penalized-AoO rule — the consumer that feeds it rides with P05's maneuver UI), ready-before-trigger ordering (D-193's rank; P07 closed at D-195), and now the situational fold. Evidence: unit 1929/3 skipped, e2e 125/125.
- [x] **P07 — Complete delay/ready execution and UI:** triggers, interrupt resolution, initiative adjustment, unused/lost actions and re-ready; prevent extra-turn/action exploits. (G §4.11; I P2/P6) — **partial 2026-09-12 (D-193):** the state and legality of both actions now exist as pure transitions. `pf1e/readyDelay.ts` (new) owns `delayTo` (permanent lower-count initiative, re-sort, `combat.turn` re-pointed at the next actor), `readyCombatant` (standard action spent, `flags.pf1e.ready` stored, prepared action restricted to standard/move/swift/free — never full-round), `resolveReady` (initiative becomes `triggerer.initiative + 1`, ready spent, turn pointed at the readied combatant so it acts now, prepared action returned; refused on initiative collisions rather than silently misplacing), and `findReadied` (trigger matching by kind + optional `targetId` scope). `readCombatantState`/`PF1eCombatantState` gained the defensive `ready` round-trip, and `pf1eNextTurn` clears an unspent ready at the combatant's next turn (`sinceRound < core.combat.round` — unused ⇒ lost, re-ready allowed). The interrupt queue gained a second `PF1eInterruptKind` (`readied-action`) with `queueReadiedAction` and a ready-before-AoO rank in `orderInterrupts`, closing P06's "ready-before-trigger ordering". Exploit guards are structural: both transitions re-point `combat.turn` so a delayer/readied combatant is never revisited in the round it left (a test walks the whole round and asserts the wrap). 24 new tests. **Remaining:** the CombatPanel wiring (a real Delay/Ready control replacing the metadata-only "Mark delayed" button, plus the GM trigger-evaluation prompt), and effect-duration ticking at the moment of a delay (the UI slice routes delay through core's tick). P07 stays unchecked. **Second slice 2026-09-12 (D-194):** that UI now exists. `CombatPanel.svelte` gained Delay / Ready / Fire-ready controls on the current combatant's row (the metadata-only "Mark delayed" button survives only for non-PF1e encounters, where core's marker is still correct), each routing its result through the same `push` every tracker control uses — refusals surface as the panel error verbatim. Delay ticks the delayer's own effect durations via a new `endTurnEffects` in `core/combat.ts` (the existing `tickEffects` core, repackaged to end one turn without advancing the round), and `ReadyDelayTransition` is now literally core's `CombatTransition` (it gained `expired`), so `push` accepts a delay unchanged. The GM is the trigger-evaluator (the SRD leaves the trigger to the table): inline forms take the action/trigger/target/note, a `readied` badge shows on a combatant carrying `flags.pf1e.ready`, and firing a ready sets a status line and moves the combatant immediately ahead of the triggerer. +1 unit test (the delay tick) and a new **executed** browser spec `e2e/pf1e_ready_delay.spec.ts` (delay reorders and hands off the turn, ready spends the standard and badges, fire-ready moves the combatant to triggerer+1 and clears the badge); `e2e/combat.spec.ts`, `e2e/sheets.spec.ts` and `e2e/pf1e_opportunity.spec.ts` all still pass. P07 stays unchecked. **Third slice 2026-09-13 (D-195):** the fired readied action now **resolves** through the sheet's own attack flow instead of a manual GM instruction. `ui/combat/pf1eReadyAction.ts` (new) orchestrates it — `resolveReady` first (reorder + spend + hook), then a readied **standard attack** runs `resolveAttackFlow` against the triggerer as the combatant's primary **melee** attack (`meleeLine`, exported from `pf1eAooFlow.ts`), labelled "— readied action", normal defense (no flat-footedness), no budget spend (the standard was already paid), HP write + card inside the same resolver the AoO path uses; the `PF1eDerivedAttack`-group reuse comes from exporting `actorOf`/`featsOf`/`authoredAttackCount`/`derivedFor`/`meleeLine`. Move/swift/free readies, missing scene/token/actor docs, and ranged-only combatants each **hand off by name** (the reorder stands, the GM finishes the combatant through the normal flow). `CombatPanel.svelte`'s `doFireReady` awaits the resolver and writes `firedNote` ("…'s readied standard action fires — <attack line>", or the D-194 hand-off line). 4 new unit tests (`tests/ui/pf1eReadyAction.test.ts`) + a new **executed** browser spec `e2e/pf1e_ready_fire.spec.ts` (2 tests: a real-d20 hit moves HP with `damage = hpBefore − hpAfter` and posts the "— readied action" card; a ranged-only ready hands off) via a new `pf1eReadyFire` e2e surface. The fire form now also **surfaces the trigger matcher** for GM confirmation: it declares the event (kind + combatant to interrupt), runs `findReadied` live, and shows "✓ matches — fires just before X" (`data-fire-match`) or "✗ armed for cast, not this attack event" (`data-fire-mismatch`), asserted in `e2e/pf1e_ready_delay.spec.ts`. **Verified complete 2026-09-13:** every checklist item is implemented and tested — triggers (`findReadied` kind + `targetId` scope, unit-tested and now surfaced in the tracker), interrupt resolution (`resolveReady` reorders just-before-the-trigger, `resolveReadiedAction` resolves the standard attack through `resolveAttackFlow`, and the queue ranks `readied-action` ahead of every AoO), initiative adjustment (`delayTo` permanent lower count, `resolveReady` triggerer+1), unused/lost actions (`pf1eNextTurn` clears a carried-over ready), re-ready (allowed once lost), and the exploit guards (the delayer is visited exactly once per round; initiative-collision fires are refused). The one SRD detail not encoded — "a 5-foot step may accompany a ready" — is the caller's movement-ledger spend, not this module's (documented in `readyDelay.ts`); the 5-foot step itself is a separate action-economy item. Evidence: full suite **1826 passed / 3 skipped** across 162 files; typecheck/lint green; build 2,315.62 kB / 672.40 kB gzip; e2e ready_delay (1), ready_fire (2), combat (3), sheets (10), opportunity (20) all pass.
- [x] **P08 — Add mount/companion linkage and mounted rules:** shared initiative/space, Ride DCs, movement/full-attack limits, mounted ranged penalties, charge/lance multipliers, casting concentration, falls/unconscious riders and supported mounted feats. (I P6; G Appendix A.11) — **done 2026-09-14 (D-217):** rule layer closed; remaining scene/combat glue (shared 2×2 space / initiative statement) is display-only — **partial 2026-09-13 (D-201):** the rules and the linkage exist. `pf1e/mounted.ts` (new, pure) transcribes A.11 with every die the caller's: the higher-ground bonus (+1 melee vs a smaller-than-mount, on-foot foe — the situational `higherGround` flag), the full-attack bar (mount > 5 ft ⇒ one melee attack), the ranged penalties (−4 double-move / −8 run), `LANCE_CHARGE_MULTIPLIER` ×2, the mounted casting DCs (10 + SL moved-both-halves; 15 + SL running), the Ride checks (control 20 / knees 5 / stay in saddle 15) and the unconscious-rider saddle chance (50% / 75% military). The **linkage** is authored (`system.pf1e.mount` = actorId/combatTrained/saddle, normalized by `mountLinkageOf`) through a real sheet editor (a `mount` DetailEdit kind — self-ride refused, idempotent clear — plus the Mount select/trained/saddle block on the combat tab), and the sheet's resolve paths fold `higherGround` when the linked mount outsizes the on-foot target, with a `data-pf1e-mounted-bonus` hint. 10 + 3 unit tests, 1 browser test **executed**; also fixed pre-existing A06b flake (a crit could take the 12-HP dummy negative and the HP-line regex did not admit `-`). **Remaining:** the tactical consumers — Ride checks in the movement gate, the melee full-attack bar in the attack flows, the mounted cast DC into `pf1eCastFlow`, the shared-initiative/space combat wiring (Large mount = 2×2), falling/unconscious riders, and the mounted feats (Ride-By ordering; ×3 spirited charge as a caller fact). **Second slice 2026-09-14 (D-215):** ranged penalties (−4 double / −8 run) now fold into `resolveAttackFlow`/`resolveManyshotFlow` (`mountMovement` ⇒ `mountedRangedPenalty` ⇒ bonus + rewritten formula + card chip, sheet `Mount movement` select + `data-pf1e-mounted-penalty` hint, melee ignores; +4 unit/flow tests). **Third slice 2026-09-14 (D-216):** melee full-attack bar (`mountMovedFt > 5` ⇒ one melee attack, `resolveAttackFlow` refuses `iterative≠0` before any die, 5 ft allows/6 ft bars, ranged ignores; `PF1eActorSheet` `mountMovedFt` input + `data-pf1e-mounted-melee-bar` hint, inferred 0/30/60/120 from `mountMovement`) + mounted casting concentration (`castMountMovedBeforeAndAfter`/`castMountRunning` ⇒ `mountedCastingConcentrationDC` 10+SL/15+SL via Table 9-1 `vigorousMotion`/`violentMotion`, pushed into `pf1eCastFlow` `declarations`) with `data-cast-mount-*` + `data-cast-mount-dc` note; +6 tests (now 15 in `pf1eMountedFirearmsFlow`). **Fourth slice 2026-09-14 (D-217):** Ride panel (`data-pf1e-ride` with ride mod/d20/d100, DC20 untrained / DC5 knees / DC15 stay-in-saddle + 1d6 falling via `client.roll` → HP write → `pf1eSheetEdit`, DC50/75 unconscious), lance charge multiplier (`lanceChargeMultiplier` 2/×3 Spirited — `combinedMult=1+(crit-1)+(lance-1)` CRB p.179 additive) and mount footprint (`mountFootprint` Large 2×2/10 ft) with `data-pf1e-mount-footprint`/`data-pf1e-mount-initiative`/`data-pf1e-lance-hint`/`data-pf1e-lance-resolve` hints; +3 tests (now 2168 passed/3 skipped).
- [x] **P09 — Complete firearms independently at both scales:** early/advanced touch windows and max range, ammo/capacity/loading/provoke, weapon-owned broken/misfire state, penalties/escalation/Gun Training/nonproficient loading, early explosion saves/destruction and clearing actions. Reverify questionable G firearm formulas before encoding; measure any new strategic state columns. (G §2.9/2.9b; I P6/D-113 normalization notes) — **done 2026-09-14 (D-219):** the misfire rules exist and fold into the real resolve path. `pf1e/firearms.ts` (new, pure) encodes §2.9b with the clearing texts re-verified (the hedged "DC 10 + ?" generic clear does **not** exist — Quick Clear is a standard action needing ≥1 grit, 1 grit makes it a move action; Gunsmithing is 1 hour): `effectiveMisfireValue` (minimum +4 broken, +2 with Gun Training, +4 nonproficient loader), `pf1eMisfireVerdict` (the natural face only, never a confirmation roll; nat 20 never misfires; auto-miss; first misfire breaks; a second misfire of a broken **early** firearm explodes — chosen-corner burst, DC 12 Reflex half, nonmagical destroyed/magical wrecked; advanced never explode; Expert Loading spends 1 grit to avert), and `firearmShotAmmo` (no shot loaded ⇒ impossible to attack with). Authored attack lines carry `firearm`/`broken`; the derivation exposes `PF1eDerivedAttack.misfire`; `pf1eResolveAttack` checks misfire **first** (a forced miss that cannot threaten, verdict returned for the caller's state write) and `resolveAttackFlow` reads the line + feats (Gun Training) + per-shot caller facts. 10 + 4 + 2 unit tests, 1 browser test **executed** (a broken musket at effective value 24 explodes through the sheet's resolve panel; bounded retry for the real d20). Touch windows/max range were already `tactical.ts`'s (D-113). **Remaining:** the tactical consumers (explosion burst geometry + Reflex saves, grit accounting), the load-provoke trigger through the action seam, the strategic weapon-state columns (M01's mirror), and the misfire readout panel polish. **Second slice 2026-09-14 (D-215):** ammo gate (0 ⇒ refusal before any die, §2.9) + broken/destroyed persistence (`attackerActor`/`attackerAttackIndex` ⇒ `system.pf1e.attacks.${idx}.broken:true`, idempotent, nat 20 never writes) now live in `pf1eResolveFlow` with card notes verbatim; early second misfire explodes `DC 12 Reflex half` / advanced never explodes; 9 new flow tests (6 firearms). **Third slice 2026-09-14 (D-218):** grit ledger (`system.pf1e.grit` current/max), ammo capacity/loaded per attack line, `load-firearm` provokes `yes` (Table 7-2 UC p.135), reload via `resolveActionProvokes`, `quickClearReloadCost`/`firearmExplosionSquares`, sheet Grit + Firearm actions. **Fourth slice 2026-09-14 (D-219):** strategic mirror 15 cols (`ammo:u8, weaponState:u8`, 18 B sys, 68 B/model ≤200 B, manifest synced, `deploySeed` firstSeed guard, `combatEngine` ammo gate + `weaponState|=1` + nat20 guard), burst flow `resolveFirearmExplosionFlow` (DC12 Reflex half per target, 4 squares from chosen corner, floor), sheet `resolveExpertLoading` + explosion panel `data-pf1e-explosion-actions`/`doFirearmExplosion` with `misfireExtras` pass-through. Touch already `tactical.ts`. Tests `pf1eFirearms` 10→15, `pf1eCombat` ammo seeded, `e2e/pf1e_firearms` D-202 24-value + D-219 pure 4 Chromium + UI reload 1→0→reload→1 provokes (pure Chromium shard passes, UI needs browser binary — D-189 precedent, `vite build` proves sheet). Budget `dist/index.html` ~2,463 kB raw / ~711 kB gzip (budget 6 MB).

### P6/P8/P09 progress — 2026-09-14, step-3 audit and queued wiring (D-215)

- **Candidate p2_p3 = Finish 2 then do 3 — step 2 closed, step 3 re-verified 2026-09-14:** A07 + C03 checked above (D-212/D-213). Remaining tactical work is exactly the plan's #3 — **P05's companions already done** (D-214), leaving **P08 mounted** and **P09 firearms** pure layers already landed and their tactical consumers queued.
- **P08 re-verification (mounted, 222 lines `pf1e/mounted.ts`):** `mountLinkageOf`/`mountedHigherGround` (sizeSteps < 0 → +1 `higherGround`), `mountedMeleeFullAttack` (>5 ft → single melee only), `mountedRangedPenalty` (−4 double / −8 run), `LANCE_CHARGE_MULTIPLIER` ×2, `mountedCastingConcentrationDC` (10+SL moved both halves / 15+SL running), Ride DCs 20/5/15 and 50/75% stay-in-saddle, linkage authored via `system.pf1e.mount` and sheet Mount block → `situational.higherGround` already folded into `resolveAttackFlow`/`resolveManyshotFlow` with `data-pf1e-mounted-bonus`. **Queued wiring:** Ride/movement/full-attack/ranged/cast consumers (movement gate, attack-flow melee bar + ranged penalty, `pf1eCastFlow` mounted DC) plus shared 2×2 initiative/space, falls/unconscious riders, mounted feats (Ride-By ordering, ×3 spirited charge as caller fact).
- **P09 re-verification (firearms, 227 lines `pf1e/firearms.ts`):** `effectiveMisfireValue` (+4 broken / +2 GunTraining / +4 nonproficient loader), `pf1eMisfireVerdict` (nat 20 never misfires, first breaks second early explodes DC 12 half unless expertLoading, advanced never explodes), `firearmShotAmmo` (0 → refusal), `MISFIRE_CLEARS` (QuickClear standard→move with grit / Gunsmithing 1 hr), touch already in `rangedAttackRange` firearmTouchIncrements. **Queued wiring:** ammo gate + broken/destroyed write + grit + explosion burst (Reflex saves) + load-provoke through `pf1eActionOpportunity`, strategic columns (M01 mirror), sheet misfire readout.
- **Evidence for pure layers:** 10+3 mounted / 10+4+2 firearms unit tests, 1 browser test each executed (D-201/D-202), plus the sheet's `mountedHigher` and misfire-first checks already live in the resolve path. P08/P09 stay unchecked pending the queued consumers and their Chromium e2e, per the D-119 precedent.

### P6/P8/P09 progress — 2026-09-14, third slice (D-216)

- **D-216 — the last two caller-fact gates that already had pure layers now reach the sheet/flows:** `resolveAttackFlow` takes `mountMovedFt` and refuses `iterative≠0` melee when `>5` before any die (`only one melee attack at the end of the move, no full attack (A.11)`; 5 ft allows, 6 ft bars, ranged ignores), with `PF1eActorSheet` wiring (`resolveMountMovedFtRaw` + derived `resolveMountMovedFt` inferred 0/30/60/120 from `resolveMountMovement`, `mountedMeleeBar` hint `data-pf1e-mounted-melee-bar` beside the ranged penalty) and the param threaded into `resolveVsTarget`; and `PF1eActorSheet`'s cast gate pushes `vigorousMotion`/`violentMotion` (running wins, matching `mountedCastingConcentrationDC`) into `pf1eCastFlow` `declarations` via `castMountMovedBeforeAndAfter`/`castMountRunning` checkboxes (`data-cast-mount-moved`/`data-cast-mount-running`) and a `data-cast-mount-dc` note (10+SL / 15+SL reuse of Table 9-1, no new `concentration.ts` situation). Both gates are caller-owned, carry no dice, and were already tested as pure helpers (`mounted.ts` 13 tests + `concentration.ts` 29) — the slice only wires caller facts. +6 tests in `tests/ui/pf1eMountedFirearmsFlow.test.ts` (now **15**): melee `iterative:0` at 30 ft allowed vs `iterative:1` refused with the A.11 wording and no rolls, 5 vs 6 ft boundary, ranged ignores; `mountedCastingConcentrationDC` 3→13/18 and `concentrationDc` vigorous 13/violent 18.
- **Remaining P08 (after D-217):** only the combat/scene wiring (mount and rider share the mount's 2×2 space and initiative — already displayed via `data-pf1e-mount-footprint`/`data-pf1e-mount-initiative`) remains for a dedicated scene/combat glue slice if needed; the rule layer is done. P09 now queues grit, burst geometry/Reflex, clears, load-provoke, strategic columns and readout polish (D-215 left the gate + broken write wired).

### P6/P8/P09 progress — 2026-09-14, fourth slice (D-217)

- **D-217 — Ride checks, lance ×2/×3 additive and mount footprint reach the sheet/flows:** `PF1eActorSheet.svelte`'s Mount block gains the Ride panel (`data-pf1e-ride` with ride mod/d20/d100, DC20 untrained / DC5 knees / DC15 stay-in-saddle + 1d6 falling via `client.roll`/`client.store.getAll` poll → `pf1eSheetEdit` HP, DC50/75 unconscious) and the lance/footprint hints (`mountFootprint` Large 2×2/10 ft → `data-pf1e-mount-footprint`, initiative line `data-pf1e-mount-initiative`, lance hint `data-pf1e-lance-hint`/`data-pf1e-lance-resolve`); `pf1eResolveFlow.ts` detects a lance (`/lance/i` + charging + linked mount + Spirited Charge) and computes `combinedMult=1+(crit-1)+(lance-1)` CRB p.179 additive (×3/×4/×5 vs multiplicative ×4/×6), repeating the base damage formula and scaling `featDamageDelta`. Pure tests: `lanceChargeMultiplier` 2/3, `mountFootprint` table, additive 1+(a-1)+(b-1). Unit 2168/3 skipped, typecheck/build green, `dist/index.html` 2,460.63 kB.

## 9. P7 — Injury, recovery and death

Depends on damage/effects/turns and corrected S5 rules from R02.

- [x] **H01 — Implement disabled/dying/stable/dead progression** with negative-HP bookkeeping, Constitution-based stabilization, Heal assistance, ongoing turn effects, natural recovery and helpless/coup-de-grace resolution. (I P7; G §4.9/Appendix A.13) — **done 2026-09-13 (D-203 → D-206):** the verdicts exist and the resolve annotations now read them. `pf1e/injury.ts` (new, pure) owns every A.13 verdict with caller-supplied dice: the lethal ladder (`injuryStateOf`: healthy/disabled-at-0/dying/dead-at-negative-Con, its notes the exact card strings `resolve.ts` printed), the nonlethal thresholds (`nonlethalStateOf`: exactly-equal staggers, exceeding knocks out, lethal dominates), the disabled economy (`disabledAfterAction`: one move or standard, never both, never full-round; the strenuous standard costs 1 HP after the act — −1 and dying, no check), the DC 10 Constitution checks (`stabilizationCheck`: penalty equal to negative HP, nat 20 automatic, failure loses 1 HP; the same roll is the stable character's hourly wake), first aid (`healFirstAid`: DC 15 Heal), and the coup de grâce aftermath (`coupDeGraceVerdict`: mandatory Fort DC 10 + damage or death, the damage-killed case moot, crit immunity waives damage and save). `resolve.ts`'s condition annotations now come from the module (behavior-preserving — the string-pinning tests pass unchanged). 10 unit tests. The dying round is wired end-to-end (**2026-09-13 (D-205)**): `pf1eNextTurn` reports the dying combatant (`dyingChecks`, D-204) and the CombatPanel rolls the DC 10 Constitution check publicly (1d20 + Con mod, the negative-HP penalty inside the verdict), then writes the outcome through `ui/combat/pf1eDyingTick.ts` — the `Stable` condition on a success (the transition's own gate reads it back, so stabilized creatures owe nothing on later turns), −1 HP on a failure with `injuryStateOf`'s death annotation past negative Con — with the verdict on the `data-dying-note` status line; e2e covers the deterministic Con-35 case end-to-end incl. the no-second-roll gate. **Closed 2026-09-13 (D-206):** the promised UIs now exist — helper/target first-aid (DC 15 Heal, standard action that provokes, `ui/combat/pf1eFirstAid.ts` → `Stable` op), helpless-target coup delivery (`ui/combat/pf1eCoupGrace.ts` → HP write with −Con death line and the mandatory Fort save at DC 10+damage), and rest-based recovery (H03's `ui/combat/pf1eRest.ts` + `PF1eActorSheet` `data-pf1e-rest` panel with level/bed-rest/long-term-care rates, 1/level/night, 2× bed rest, doubled again). CombatPanel drives both combat UIs through the existing provoke/roll/host plumbing; the sheet drives rest through the same allowlisted op path.
- [x] **H02 — Complete nonlethal and temporary HP semantics:** separate accumulation, staggered/unconscious thresholds, excess conversion, absorption/stacking/expiry and lethal/nonlethal healing relationships. Do not equate tactical 0 HP with strategic compaction death. (G §2.12/§4.9; I P7) — **done 2026-09-13 (D-206):** nonlethal thresholds live in `pf1e/injury.ts` (`nonlethalStateOf`, lethal dominates), temp-HP stacking owns `system.pf1e.tempHpSources` (Record<sourceId, amount>) via `src/packages/pf1e/tempHp.ts` — same-source keeps the highest only, different sources stack, legacy scalar collapses to one map entry, deterministic `largest-first alphabetical` absorption (e.g. {b:6,a:8,c:6} after 10 ⇒ {b:4,c:6}), `expireTempHpSource` deletes one source, `healingDoesNotRestoreTempHp` enforced; `src/packages/pf1e/healthState.ts` exposes the map and total, `resolve.ts` (single + Manyshot) and `pf1eResolveFlow.ts` absorb before HP, `CombatPanel`/`PF1eActorSheet` surface grant/expire (`data-pf1e-temp-hp-panel`) and the `d.tempHp · entries` readout. 14 new tests in `tests/packages/pf1eTempHp.test.ts` incl. the largest-first fixture and resolve absorption on hit/partial/miss/legacy.
- [x] **H03 — Add healing, fast healing and regeneration** including suppression sources/timing and death implications; ensure damage/healing state and analytics agree. (I P7; M Tasks 3/6; G Appendix A.18) — **done 2026-09-13 (D-206):** `src/packages/pf1e/healing.ts` (`applyHealing` caps at maxEffectiveHp and removes equal nonlethal per CRB p.191/AoN 170, `applyNonlethalHealing`, `fastHealingTick`/`regenerationTick` with explicit `suppressed` boolean — suppressed regeneration heals 0, dead never heals), `src/packages/pf1e/combatEngine.ts`'s `resolvePF1eHealing` books both `pool.hp` and `pool.sys.nonlethal` through it with −Con death, `PF1eActorSheet` healing (`data-pf1e-healing`) and natural-recovery (`data-pf1e-rest` → `recovery.ts` rates: 1/level/night, 2× bed rest, doubled again with long-term care; ability damage 1/2 per night, drain never heals) plus the `ui/combat/pf1eRest.ts` planner; `resolve.ts`'s nonlethal max-HP conversion already suppresses when regeneration is active. 11 new tests in `tests/packages/pf1eHealing.test.ts` incl. the 20/20+8 nonlethal → 20/2 case and the suppressed-regen 0 fixture; `tests/packages/pf1eRest.test.ts` (5) covers the rest planner.
- [x] **H04 — Add ability damage/drain and energy drain/negative levels**, recovery and derived-stat consequences. Ability burn/massive damage remain explicit optional follow-ups below unless separately approved. (I P7; G §4.9) — **closed 2026-09-13 (D-207):** negative levels and natural recovery exist (ability damage/drain derivation was already in). `pf1e/negativeLevels.ts` (new, pure) transcribes AoN 427 + the Energy Drain UMR: −1 per level on attacks/saves/skills/ability checks/CMB/CMD, −5 current **and** total HP per level, one level lower for level-dependent variables (spell slots and prepared spells are NOT lost), death at levels ≥ authored Hit Dice; the removal rhythms as named kinds — temporary (a new save each day at the causing effect's DC), energy-drain (one Fort save after 24h, DC 10 + ½ racial HD + Cha, a separate save per level, failure ⇒ permanent), permanent drain (no save; restoration only, a caller fact). `pf1e/recovery.ts` (new, pure) transcribes AoN 170 (CRB p.191): HP 1/level per night (2× for complete bed rest), ability damage 1 point per affected score per night (2 for bed rest; drain never heals naturally), long-term care doubling both. `pf1e/energyDrain.ts` (new, pure) closes the infliction/restoration/24h-save arithmetic: temporary vs permanent infliction, `any` restoration preferring temporary, one save per level with the becoming-permanent conversion, and the derivation's `negativeLevelTotalsOf`/`negativeLevelSaveVerdict` as the single source of dice. `src/ui/combat/pf1eEnergyDrain.ts` (new, pure plan) produces the exact `actors` ops (`system.pf1e.negativeLevels` or its `-=` deletion) with card notes. `PF1eActorSheet.svelte` `data-pf1e-negative-levels` surfaces the live totals/penalties/death line and inflict / save (die+Fort+DC) / restoration controls (all owner-gated, whole-number-validated, planner-delegated). The derivation folds authored `negativeLevels` into every listed statistic and pushes `dead` at the HD threshold; `pf1eNextTurn` reports `dyingChecks` (the dying round's stabilization obligation, D-203's `stabilizationCheck` the consumer). 7 + 2 + 3 + 2 + 12 + 8 unit tests; build 2,385 kB raw / 692 kB gzip. Ability burn/massive damage remain the explicit optional follow-ups. The dying-check surface itself closed in D-205, and the combat UI's first-aid/coup/rest pieces closed in D-206.
...
- [x] **M01 — Bring strategic attack/damage rules up to the verified data contract:** outstanding G §2.4–2.10 (crit/bonus dice, ranged Dex/size, handedness/enhancement, range/firearms and defender mitigation) using independent sim loops; add scale-specific fixtures, not a cross-scale equality gate. (G §§2, 5; I P8) — **landed 2026-09-15 (D-227):** every §2.4–2.10 row is now data-driven in `compilePF1eProfile`/`resolvePF1eAttacks` and pinned by 25 scale-specific fixtures in `tests/packages/pf1eAttackFidelity.test.ts`. §2.4 crit × multiplier applies only to base dice + static bonuses; bonus dice (energy/precision) roll once. §2.5 Improved Critical doubles the threat width (19–20→17–20). §2.6 ranged attacks use Dexterity and the attack/AC size modifier now reaches the die; the CMB/CMD *special* ladder is its own `specialSizeMod` field so the two signs can never collide. §2.7 handedness Strength shares (1.5× two-handed / 0.5× off-hand, penalties whole; thrown keeps Str to damage) plus enhancement-to-damage. §2.8 one range rule for every weapon — −2 per full increment past the 1st, legality refused beyond max increments (thrown 5 / projectile 10) **before** an attack is counted or a die consumed. §2.9 firearm classes: touch window early ≤1 / advanced ≤5, then standard AC; ceilings 5/10. §2.9b broken weapon −2 attack / −2 damage. §2.10 DR: compound forms need EVERY listed quality (the first-match OR is gone), the +1/+3/+3/+4/+5 enhancement ladder covers the alignment row via the existing ALIGNMENT bit, weapon alignment flags bypass /alignment at any enhancement, and bonus dice (energy + precision) ignore DR entirely; `/epic` is refused with reason (no mythic content, the u8 byte is full — DEVIATIONS). Authored weapon payloads flow from numeric `stats` keys through `rawProfileFromUnit` (deploySeed) into profiles. Measured fixture movements, all by design: PRECREATED Large units (cavalry/troll/golem) lost +1 to hit (−1 is correct), artillery to-hit +5→+4 (Dex, not Str) and gained real range penalties, paladin hero damage +5→+7 (enhancement). Cross-scale agreement stays pinned in `pf1eActor.test.ts` (ladder, special size, saves), not re-read as an equality gate.
- [x] **M02 — Reconcile strategic compile differences** in AoO budget, CMB/CMD special size and save bases/modifiers using verified shared tables; keep legacy/stat-block conversion differences explicit and measure changed fixtures. (G §10.2; I P8) — **reconciled by audit 2026-09-15 (D-226):** all three §10.2 differences are closed and pinned. **AoO budget** uses the one shared table `rulesTables.attacksOfOpportunityPerRound` (one per round; `1+Dex` clamped ≥1 with Combat Reflexes) since D-183, with authored stat-block `maxAoos` ingested as-is (`schema.ts:289`). **Special size** — `cmbFrom`/`cmdFrom` in the shared core take the special ladder, never the attack/AC one, and a stat block publishing only `sizeMod` keeps that number via `sizeModOverride` with the deviation recorded in `converted` (`actor.ts:1004-1005,1160`); pinned by `pf1eActor.test.ts:244` ("the special size ladder and the formula, not an authored total"). **Saves** — published numbers are used as totals at both scales; the ability component is never re-added; the record lands in `converted` (`pf1eActor.test.ts:490-493`, "published saves are totals … not re-added"). Every number flows from `rulesTables.ts`; the differences stay visible per document, never averaged away.
- [x] **M03 — Fix condition/status bit collisions** with a budgeted separate column or safe allocation; update manifest, codec, spatial filters, compaction and joiner tests so prone/flanked never masquerade as hidden/pinned. (G §2.13; I P8; B §4.5) — **fixed and audited 2026-09-15 (D-226):** PF1e conditions live in the dedicated `pfCondition` u32 column (`PF1E_MODEL_SCHEMA` — the declared manifest), disjoint from `ModelPool.status`; the numerical bit reuse (PRONE==hidden 1<<4, FLANKED==pinned 1<<2) can no longer mislead because no consumer reads condition bits from `status`. Envelopment writes/clears FLANKED there only (`envelopment.ts:48-53`). `tests/packages/pf1eStatusCollision.test.ts` pins: both-bit independence, FLANKED≠pinned through `markPF1eFlanking`, PRONE models still matched by spatial queries, combat-maneuver PRONE lands in `pfCondition`, the engine grant of the +2 flank bonus only from `pfCondition`, snapshot **and** delta round-trip (codec + compaction paths), and the ≤200 B bytes-per-model budget (70–72 B with the column). Joiner fidelity is schema-generic — the N01/N02 announce/adopt flow carries every declared column, so a joiner's replica decodes `pfCondition` by the same tested codec path.
- [x] **M04 — Replace heuristic engagement with scale-appropriate contact/reach/flanking geometry** and clear/recompute stale FLANKED bits each turn; handle envelopment movement and document any non-SRD bonuses. (G §2.2/§5; M Task 4) — **complete 2026-09-14 (G-04/D-223):** the final open item — envelopment *movement* — landed as the doctrine-mode flank wrap: engaged wider fronts relocate excess files to corner-anchored flank slots (own-side only, one square per wrap rank), and `markPF1eFlanking` then judges the resulting geometry per AoN 183. Non-SRD bonus documentation: **none exists to document** — D-130 deleted the invented +4/flat-footed rule and G-04 adds none (geometry only, +2 SRD flanking through the existing rule). Facing/facing-order remains out of scope (P02 has no facing state in the sim pool). Earlier work 2026-09-12 (D-177, D-180, D-182): the invented rule is gone. reach/contact ride feet and the scene grid (D-177); per-size/reach-weapon reach comes from each unit's bound leader actor (D-180); and **the true flanking rule is now the strategic rule** (D-182): `envelopment.ts` is rewritten around `markPF1eFlanking`, which clears and recomputes every living model's FLANKED bit once per turn through `flanking.ts`'s AoN 183 `resolveFlanking` — opposite borders/corners, threatening ally read through `threatenedCells`, Table 8-4's can't-flank exclusion, enemy = different faction with an `isEnemy` override. `TurnChannel` deploys one model per scene square (P01, closed), which is what made the line test meaningful; a defender flanked by two different units is flanked regardless of engagement order; and `resolvePF1eAttacks` reads the bit itself (combatEngine.ts:271), so the +2 is the existing applier and nothing new was invented. The switchover also removes a bound bug the old engine had (a 5-ft `queryPoint` could not see a 7.07-ft diagonal contact). **Open: envelopment movement and non-SRD bonus documentation** (G §5); threatened-square adjacency is exercised through the threatened set, facing remains open with P02.
...
- [x] **M06 — Complete strategic SR/save, nonlethal, healing and regeneration paths** with correct counters and model lifecycle handling; retain independently seeded deterministic resolution. (G §2.11–2.12/§5; I P5/P7/P8) — **partial 2026-09-12 (D-176):** fast healing and regeneration now execute once per turn in a `heal` sub-phase (SRD "at the start of its turn"; capped at the effective maximum; living models only; `damageHealed` finally booked), authored via unit stats and consumed through the tested `resolvePF1eHealing` engine. Regeneration suppress-types/can't-die semantics verified already complete at the strategic layer (fire/acid → lethal tally → death; other hits → nonlethal + DISRUPTED at 0, heal phase re-rolls it). **closed 2026-09-15 (D-228):** strategic nonlethal matches the A.13 thresholds — equal to current HP sets the new STAGGERED bit (one attack per resolution routine at this grain), beyond sets UNCONSCIOUS (which now skips attacks outright), and the past-max excess converts to lethal through the attack's leftover DR; SR now routes through the tactical-scale `spellResistanceCheck` with a per-turn overcome cache (`srRoundCache`), retiring the strategic nat-20/nat-1 house rules. `pf1eNonlethalFidelity.test.ts` (8).

### Player hero bridge

- [x] **M07 — Produce real hero identity and actor inputs:** `leaderTokenId` → actor → profile/`isHeroUnit`; populate `RulesContext.leaderActors` at production construction sites instead of empty maps or fixture IDs. (G §4.12/§5; I P8; M Task 7) — **partial 2026-09-11 (D-168/D-169):** `collectLeaderActors` populates `leaderActors` (keyed by unit id) at both production sites; the strategic caster's CL/key-mod/Spell Penetration ride the leader actor via the shared tactical derivation; and `isHeroUnit` now also recognizes the leader-actor binding (unit id present in `leaderActors`), so a bound unit radiates the leadership aura and, since D-178, cleaves per the SRD feat — one extra attack at full BAB against an adjacent enemy, with the −2 AC penalty. Leadership auras use the work plan's fixed radius/bonus (R02 in D-172 proved the SRD Leadership feat has no aura to author from); `heroBridge`'s query radii were unit errors, corrected in D-172. The cleave's literal `damageDealt: 25` and the rejected overkill-cascade model were **deleted in D-178** (Gap List §5 closed for cleave). **closed 2026-09-15 (D-229):** hero attack/defense stats (hp, move, AC-touch, DR, SR, saves, BAB, Str/Dex) ride one tactical `deriveFromDocuments` call into `unit.stats` on advance, persist through the same atomic resolve envelope as battle diffs, and the binding itself is authored from the ArmyWindow roster leader select (token → linked actor); the player's own sheet stays the sender the queue always relied on; a movement-during-turn reconciliation test (M08's last open line) passes.
...
- [x] **M09 — Fix aura application/removal and stacking:** radius/bonuses from feat/class data, spatially eligible allies only, no per-model/per-turn accumulation into base stats; leaving formation or losing the leader removes benefits. (G §5; B §4.1; M Task 7) — **closed 2026-09-15 (D-230):** radius/bonus now ride authored unit stats (`leadershipRadius`/`leadershipMoraleBonus`, D-172 proved the SRD feat authors neither); the aura anchors on the leader's first *living* model and dead-leader radiation is blocked at the bridge, so losing the leader or leaving formation removes benefits in the same turn; stacking is impossible because `seedPF1ePool` rewrites saves/AC before auras each turn. `pf1eHeroBridge.test.ts` +3.
...
- [x] **M11 — Populate all advertised metrics at their real event sources:** attacks/hits/misses/percentage, damage dealt/taken/overkill, threats/confirms, kills/deaths/remaining, DR absorbed/SR blocked, saves, AoO executed/hits, CMB success, healing/regeneration/channel-energy counts. Specifically close the six unincremented fields identified by I P8. (M Task 6; G §5; I P8) — **closed 2026-09-15 (D-226) on top of D-165/D-170/D-176:** the six I P8 fields each have a real source — `srBlocked` from the per-model SR roll (`spells.ts:239`), `aooExecuted`/`aooHits` from the defensive-cast AoO armed in mass battles, `deathsCount` booked on the unit that lost models (`recordSpellKills` + `recordCombat.kill` D-165), `damageHealed` from the heal sub-phase (`recordHealing`, massBattlePf1e.ts:978); `cmbSuccesses` stays refused-with-reason (no mass-battle maneuver mechanic exists to source it — the field is kept initialized, not fabricated). The broader advertised list all books through source-resolvable paths: attacks/hits/misses/percentage and threats/confirms from the combat loop, DR absorbed/bypassed from the DR branch, saves passed/failed from the spell save leg (`spells.ts:254`), concentration from the concentration path, kills from the engine. Overkill is not tracked as a counter **by design** — D-178 deleted the overkill-cascade model; `netDamageDealt` is the honest aggregate. Channel energy likewise has no mass-battle mechanic or field to populate — nothing advertises it. Reconciliation into every turn report is M12's landed work; CSV export is M13; the browser rendering is M14. — **partial 2026-09-11 (D-165/D-170):** strategic spells attribute hits per owning unit on the spell event (`hitsByUnit`) and book `deathsCount` on the unit that lost models; defensive-cast AoO is now armed in mass battles, so `aooExecuted`/`aooHits` have a real source (threat list → `resolvePF1eAoO` → `PF1eSpellMetrics` → `recordSpell`); the dead combat-side `srBlocked` was deleted (summary field stays, fed by spells); the collector lives for the module's lifetime, so `forecast()` returns real `generateReport()` totals. Open/refused-with-reason: `cmbSuccesses` has no mass-battle mechanic to source it (no maneuvers) — field kept, initialized. Update 2026-09-12 (D-176): `damageHealed` now has a real source — the heal sub-phase books fast-healing/regeneration via `recordHealing`.
- [x] **M12 — Call `generateReport()` from actual turn resolution**, reconcile per-model/per-unit totals into TurnReport summaries, and test resets/aggregation/attribution rather than collector-only fixtures. Done 2026-09-14 (D-224): `SimRunnerCore.resolve` calls the module's `forecast()` (which performs the real `generateReport()` on the same instance that resolved the turn) for every army and reconciles the per-army cumulative sheets into `TurnReport.summary.analytics` — every turn report replicates the running campaign-aggregate totals. `projectReportForFaction` gained the `visibleArmies` redaction so a player's projection keeps only armies it can see (GM gets the full sheet). `tests/sim/runnerAnalytics.test.ts` pins accumulation across resolved turns, per-army attribution with asymmetric builds, the no-forecast/empty-armies opt-out, player redaction, and replay determinism (4 tests). (M Task 6; G §5)
...
- [x] **M14 — Mount ArmyWindow, PF1eBattleAnalysis and TurnReportTimeline** in normal WindowHost navigation, not just e2eHook. Done 2026-09-14 (D-224): GM toolbar's **Armies** button opens `kind:"armies"` in the standard window manager chrome (`ArmiesTab` picker) → clicking a card opens `kind:"army"` (`ArmyWindow`) with the rules module resolved from the campaign's ACTIVE system package (`armyWindowRules`, not the e2eHook's hardcoded mass-battle-basic); ArmyWindow's new **analysis** tab mounts `PF1eBattleAnalysis` fed from real turn reports (`analysisReportFromReports` rebuilds the worker collector's full sheet from `summary.analytics`, totals reduced, RFC-4180 CSV via its own export button), `TurnReportTimeline` rides the reports tab as before. Chromium e2e: armies window mounts through normal navigation with honest empty state + chrome controls. (G §1.7; I P8; M Task 8)
...
## 13. New Features — 2026-09-14 — GM Roll Ledger & Simultaneous Strategic (User Request)

Two user-requested tactical/strategic features that sit on top of the landed P0–P09 stack. Both reuse the existing host-authoritative Op/Msg/ModelPool contracts; neither invents a second rule engine. ~~`combat_resolver_5.html` was attached to the prompt but is not present in the workspace snapshot~~ — **the file has since landed at `Examples/combat_resolver_5.html` (`main` `14a1a77`) and the verbatim reconciliation is complete (item 5 below, D-223, 2026-09-14)**: the spec below is the reverse-engineering it was verified against. G-04's doctrine/envelop/army-initiative settings (D-223) build the file's parallel per-army execution, movement-as-unit and enveloping logic on top of F02.

### F01 — Tactical Roll Ledger: representation + rollback (non-strategic game)

**Goal.** Every roll that occurs during a turn in a *non-strategic* (tactical PF1e) game is logged as a first-class chat roll card that is human-readable, navigable and reversible for 1–2 rounds. The card is the audit trail (who → what → whom → dice → total → modifiers + damage) and the undo surface (Reroll / Revert / Player Reroll). Permanence beyond 2 rounds is explicitly out of scope — the ledger prunes, it does not become a 50-round time machine.

#### User stories

* As a GM I see *every* tactical roll (attack, save, skill, concentration, damage, rider) as a single esthetic card in chat, not a bare `1d20+5 = 18` line. I can read at a glance who initiated it (reason: attack `Longsword +8`, spell `Fireball CL 5`, ability `Trip CMB +11`), whom it targeted (single target name or area epicenter coordinates), the natural die, the total with a dropdown of every modifier and its reason, and the damage total with the same breakdown. Initiator and target names are links; clicking centers the map on that token (middle of screen) and draws an outline highlight. For an area effect the link centers on the epicenter and draws the effect outline (sphere / cone / line / cylinder / spread correctly oriented) that fades over ~4 s so the tactical map does not clutter; the fade duration is a slider in Settings → VTT (world-settings storage).
* As a GM I can press **Reroll** on any card from the last 1–2 rounds and have the system *correctly reapply* all effects: the old damage is reverted, the new damage is applied, condition Ops are swapped, a killed model is revived if the new total no longer kills, a saved-against-fireball that now fails takes the full damage, etc. **Revert** removes *all* effects of that card as if it never happened (damage healed, `prone` condition removed, fireball's `burning` expired, a model that died from that card's damage is back alive at its pre-card HP). **Player Reroll** does the same reroll but surfaces a *reroll button inside that card for the named player* — players love to roll their own dice, so the GM delegates the physical `d20` while the host still evaluates and reapplies deterministically. All three are permission-gated (GM only for Reroll/Revert/grant; player only for the delegated reroll) and refuse with a named reason outside the 2-round window, on a missing target, or without ownership.

#### Detailed requirements (acceptance = all must hold)

* **Card content — single target.** `who` = initiator actor/token name + portrait (if any) + initiator's roll-modifier snapshot (BAB, Str, size, enhancement, stance deltas, cover etc.). `what` = action label (`attack`, `spell`, `ability`, `save`, `concentration` + spell/feat name). `at whom` = target actor/token name(s) *or* for area `at (x,y) radius R shape S` with a link that centers on the epicenter. `what rolled` = natural `d20` face (or `d100`/`d6` for saves/damage) + total = die + sum(modifiers). **Modifiers dropdown** — each entry `{label, value, reason}` with the same deliberation for damage rolls (weapon dice + static + feat riders + precision + energy type). The dropdown is per-roll-type: attack modifiers vs damage modifiers are separate lists, both verbatim from `tactical.ts:attackModifierParts` / `damageModifierParts` and `resolve.ts:prepareAttack`'s situational/concealment notes, not recomputed prose. `Flavor` carries the breakdown line (A06's `flavor` right now) — the card renders it as a chip under the total.
* **Card content — area.** Shape/dimensions/origin/direction come from `targeting.ts:PF1eAreaGrid` + `pf1eAreaPreviewModel` (burst/emanation/cylinder/spread, cone/line currently refused under C01b — the card must show the C01b refusal rather than a blank shape). The link centers on the epicenter and draws the outline via the canvas `AreaPreviewLayer` / `TemplateLayer` (world-space cell rects). The outline is a *temporary* overlay, not a persisted `TemplateDocument`; it is added to a dedicated `RollEffectHighlightLayer` and removed after the fade timer. The fade is `opacity 1→0 over N seconds` where `N` is a slider in `Settings → VTT → Roll highlight fade (1–10 s, default 4 s)`, stored in `world-settings.flags.pf1e.rollHighlightFadeSec` (or `settings.flags.core` per the existing world-settings contract, 1 s clamp). While fading it must not block clicks.
* **Links + centering.** Initiator/target names render as `<a data-roll-link="actorId:tokenId">` (actorId always, tokenId when a linked token exists on the active scene). Click handler: `centerOnToken(tokenId)` → `canvasCamera.centerOn(token.position)` + `tokenHighlight.outline(tokenId, color)` (2 s outline, additive with the fading area outline for area cards). If the token was deleted, the link is disabled with `— token gone` tooltip. No new document type for the highlight; it reuses the existing `tokenBadgesFor` / `ThreatOverlayLayer` outline path.
* **Buttons — Reroll.** Host re-evaluates the *same* roll description with fresh RNG (host `roll`/`rollVerified` path, same `seedClient`/`seedHost` audit as A06). For a damage-bearing card this is *two* rolls in order (attack `1d20` then damage `NdM` / `confirm` if threat) — the button rerolls *that particular roll* the card represents, but the card owns its whole chain: rerolling a `Fireball 8d6` damage rerolls the 8d6 for *all* targets; rerolling a single attack's `1d20` rerolls only that attack while keeping the same damage formula. After the new total is known the flow does **revert-then-reapply**: (1) compute the inverse of the card's original `system.ledgerOps` (see data model), (2) validate the inverse still applies (target HP has not been overwritten by a later unrelated write that would make the revert diverge — if it has, refuse with `ledger stale — effects changed since`), (3) submit the inverse + the new effect Ops as one `OpEnvelope` (so a single `rejected` rolls the whole reroll back), (4) replace the card's `roll` + `ledgerOps` + `modifiersSnapshot` with the new values, (5) post a new card line `Rerolled: old [[X|formula]] → new [[Y|formula]]` and keep the original card's `rerollCount` + `reverted:false`. Reroll is only offered when `turnNumber ∈ [currentTurn-2, currentTurn]` (inclusive; `currentTurn` from `TurnEngineState.turnNumber` or the PF1e encounter's `flags.core.round` when not in mass battle — tactical only).
* **Buttons — Revert.** Computes the inverse of `system.ledgerOps` (the exact FlatDiff inverses `OpLog` already produces for `store.rollback`, now persisted per card) and submits it as one envelope. On success the card is marked `reverted:true`, its buttons become disabled with `— reverted` and a new system message `Reverted: <card name>` is posted. If the inverse would resurrect a dead model, the model is back alive at its pre-card HP (the `pool.hp` write is part of the inverse); its `status` bits (`dead`, `unconscious`, `prone`) are restored from the pre-image, not guessed. Outside 2 rounds, or if any `ledgerOps` target document was deleted, refuse with the same `ledger stale` naming.
* **Buttons — Player Reroll.** GM-only grant button `Delegate to player` (per card, per player) that writes `system.pendingReroll = { playerId, expiresTurn }` onto the card. The named player then sees a **Reroll** button inside *that* card (and only that player). Their click triggers the same host-evaluated reroll path but the `author` of the new roll is the player (for audit), the host still evaluates the RNG and the revert-then-reapply is identical. The GM can revoke before the player rolls (clears `pendingReroll`). A player cannot grant to another player.
* **2-round window.** `currentTurn` is the source of truth; a card authored at turn `T` is rerollable/revertible while `currentTurn - T ≤ 2`. After that the buttons render disabled with `— window closed (2 rounds)` and the server refuses even if the client forges the intent. Old `ledgerOps` are pruned: the host keeps a bounded ring of the last `2 * estimatedCardsPerTurn` cards' `ledgerOps` in memory (`OpLog` already keeps pre-images, but the card's `ledgerOps` is the *authorization* to revert — once pruned, revert is impossible by design). No migration for cards authored before this feature — they render without buttons.
* **Permissions & projection.** Reroll/Revert intents are `kind:"roll.reroll"` / `"roll.revert"` / `"roll.delegate"` messages (new `MsgKind` bytes, §13 framing, 1-byte prefix). Host validates `can(user,"update", targetActor)` for every actor touched by the ledger — same gate `pf1eSheetEdit` uses. A player without ownership of the target cannot revert damage on it. `rollMode` projection (`blindroll`/`gmroll`/`selfroll`) still redacts `roll.total` for unauthorized viewers, but the *card shell* (who/what/at whom, modifiers list *without* totals) remains visible so a blind GM roll still shows `GM → Fireball at (1200,800) — hidden` with a disabled Reroll that only the GM sees.
* **Aesthetics.** Single-column card with header (initiator portrait + name → target name(s) or `at (x,y)`), midline `[[total|formula]]` chip with disclosure triangle for modifiers, damage line `Damage [[total|formula]]` with its own triangle, and a footer row of three buttons (`Reroll` `Revert` `→ Player` with a player picker). Fading area outline is a dashed `TemplateLayer` shape, not a token highlight. Full keyboard + screen-reader labels; `data-roll-card="{messageId}"` plus `data-reroll`/`data-revert`/`data-delegate` hooks for `e2e/roll_ledger.spec.ts`.

#### Data model (no new top-level collection)

* **`MessageDocument.system.rollLedger?: RollLedger`** (new, optional, JSON-serializable, versioned `v:1`):
  ```ts
  interface RollLedger {
    v: 1;
    initiator: { actorId: DocId; tokenId: DocId | null; name: string };
    targets: Array<{ actorId: DocId; tokenId: DocId | null; name: string }> | null; // null = area
    area: { shape: "burst"|"cone"|"line"|"cylinder"|"spread"|"emanation"; origin: {x:number;y:number}; radiusFt:number; direction?: {x:number;y:number}; affectedTokenIds: DocId[] } | null;
    rolls: Array<{ kind:"attack"|"damage"|"save"|"concentration"; formula:string; total:number; terms:Json[]; seedClient:string|null; seedHost:string|null; modifiers: Array<{label:string;value:number;reason:string}> }>;
    ledgerOps: Op[]; // exact Ops submitted for this card (HP, tempHP, conditions, ledger state, spell slots) — host-authorized, replayable
    ledgerInverses: Op[]; // pre-images for single-envelope revert (from OpLog, stored so revert does not need to recompute)
    turnNumber: number; // authoring turn (PF1e round or TurnEngine turnNumber)
    reverted: boolean;
    rerollCount: number;
    pendingReroll: { playerId: UserId; expiresTurn:number } | null;
  }
  ```
  `ledgerOps` are *the* source of truth for revert/reroll — they are the same Ops `pf1eResolveFlow.ts` / `pf1eCastFlow.ts` / `touchSpell.ts` already submit, now also embedded in the card's `system` so the card is self-contained. No new `cards` collection entry is created; the existing `messages` collection carries the ledger.

* **Messages without a ledger** (old cards, pure chat) render as before — the ledger UI is `if (msg.system?.rollLedger)` only.

* **Pruning.** Host keeps `lastTurnNumber` and on each `turn.advance` / `pf1eNextTurn` wraps beyond `+2`, it issues a no-op `Op` that clears `system.rollLedger` from cards whose `turnNumber < currentTurn-2` (keeps the `content`/`roll` chip for history, frees the `ledgerOps` payload). This bounds storage without a migration.

#### Implementation steps (order matters)

1. **Ledger plumbing (pure).** `src/packages/pf1e/rollLedger.ts` (new, pure): `buildRollLedger({initiator, targets, area, rolls, ledgerOps, turnNumber})`, `canReroll({ledger, currentTurn})`, `invertLedger(ledgerOps)` (reuses `OpLog.inverse`), `pruneLedgers(messages, currentTurn)`. No store import.
2. **Host intents.** `src/core/messages.ts` add 3 `MsgKind` bytes (`roll.reroll:0x30`, `roll.revert:0x31`, `roll.delegate:0x32`), `src/host/host.ts` add handlers that validate window + `can()` per touched doc, compute new totals via the existing `roll`/`rollVerified` path, build the new `ledgerOps` via the same flow that built the old card, and submit `inverse(old) + new` as one envelope. `HostPersister` needs no schema migration — `MessageDocument.system` is free-form JSON.
3. **Flow integration.** Every tactical flow that currently does `client.submit([{kind:"create",coll:"messages",data:cardMessage}])` + `client.submit(ops)` (the `resolveAttackFlow`/`resolveCastFlow`/`touchSpell`/`pendingCast` chains) now does one envelope `[{create message with system.rollLedger}, ...ops]` where `system.rollLedger.rolls[].modifiers` is the `attackModifierParts`/`damageModifierParts`/`situational` breakdown that produced the total, and `ledgerOps` is exactly `ops`. This is a 10-line threading change per flow, not a rewrite.
4. **Chat card.** `src/ui/chat/RollCard.svelte` (new): header links → `centerOnToken`, midline `[[total|formula]]` + dropdown, damage line, footer buttons. `src/ui/chat/ChatPanel.svelte` delegates to it when `msg.system.rollLedger` exists. `src/canvas/layers/RollHighlightLayer.ts` (new, structural twin of `AreaPreviewLayer`) draws the temporary area outline and the token outline; `src/ui/settings/WorldSettings.svelte` adds the `rollHighlightFadeSec` slider (1–10 s, default 4 s) that writes `world-settings.flags.pf1e.rollHighlightFadeSec` via `worldSettingsOps` (existing seam).
...
**Goal.** Keep the existing turn-based combat (`turn.start` → `orders` → `turn.advance` → `resolution` → `report` → `turn.next`) but add a *regime* (a per-world `TurnMode`) where **all participants make their turn at the same time, in phases** — the turn is bigger because orders are collected for squads *and* units together, the GM progresses time, and then *everybody moves, everybody shoots, everybody melee's* at once. Initiative is **not** a turn-order clock in this regime; it is only the **damage-application order**: if two 100-archer units shoot at each other, the unit with initiative 12 has its damage event applied first, and the unit with initiative 7 shoots back with fewer models (and therefore fewer attacks) because some already died. This is the Combat_Resolver_5 system the user described.

*Why this is not the current mass-battle resolver.* `massBattlePf1e.ts:resolveTurn` today resolves **sequentially per attacker unit** (anchor, flanking, healing, then a loop over attacker units where each unit's attacks are rolled and damage is applied immediately). A higher-initiative unit and a lower-initiative unit that mutually target each other each fire with their *starting* model counts — the second unit does not see the first unit's casualties before it fires. Combat_Resolver_5's regime is **simultaneous issue, initiative-ordered application**.

#### Detailed requirements (acceptance = all must hold)

* **Regime toggle.** New `TurnMode` value `simultaneous` (alongside `stepwise` and `realtime` in `src/core/strategic.ts:TurnMode` and `TurnEngineState.mode`). It is a per-world setting `world-settings.flags.pf1e.strategicSimultaneous` (or `turn.mode` — same seam `worldSettingsOps` already uses for `autoResolveAoos`), default `false` (sequential) so existing campaigns are unchanged. The GM toggles it in `Settings → PF1e → Strategic simultaneous (initiative = damage order)`; the toggle is replicated and takes effect on the *next* `turn.start`.
* **Phases within one turn.** When `mode === "simultaneous"` the `orders` phase collects **all** participants' orders before `turn.advance`:
  * **Movement phase** — squads and units declare `move`/`charge`/`retreat` with `toward` and waypoints (same `OrderQueue` the tracker already edits, now for every army/faction). Walls, budget (`ctx.grid.distance`), per-model footprint and formation translation are still `massBattlePf1e.ts`'s (D-173/D-174), but the *execution* of every movement order happens **together** (all anchor translations computed from *starting* positions, then all pools updated, then the spatial grids rebuilt once). No unit's movement sees another unit's movement mid-phase; the phase's `SimEvent`s are emitted in initiative order only for the *report* ordering, not for position.
  * **Combat phase** — every unit's attack order is already declared (targetUnitId, weapon, stance). Rolls are **issued simultaneously** (all `resolvePF1eAttacks` `rng.fork(unitId)` d20s are drawn from pre-move-substream RNGs so ordering cannot alter outcomes), but **damage is applied in initiative order**. Concretely: sort attacker units by `effectiveInitiative` descending (derived `initiative` from `PF1eDerived.initiative` for the leader actor when one is bound, else the compiled profile's `initiative` / `dexMod` fallback; ties break by `total initiative modifier` then unmodified `d20` per D-123 — the same tie machinery tactical already uses, now at unit granularity). Walk the sorted list; for each attacker, compute its *current* live model count (`pool.hp` after higher-initiative damage has been applied) and derive its attack count from that (`maxIterativeAttacks`/`twfPenalties` etc. already per-model), roll its attacks, build its `SimEvent`s, and apply its `lethalDmg`/`nonlethal`/`status` writes to `pool.sys` / `pool.hp` before the next attacker in the list fires. Thus a 100-archer unit at initiative 12 that kills 20 models of a 100-archer unit at initiative 7 causes the 7-initiative unit to fire with 80 models' worth of attacks.
  * **Morale / supply / healing sub-phases** stay after combat (same `resolveTurn` sub-phase ordering `TurnReport.subPhases` already advertises: `["move","shoot","melee","morale","supply","heal"]` — simultaneous keeps the list but the engine now guarantees *all* shooters share the same `shoot` sub-phase and *all* melee share the same `melee` sub-phase; the report's `events` are still sorted by sub-phase then initiative, so the timeline reads correctly).

* **Initiative is damage order only.** Movement does **not** interleave with initiative; every unit moves regardless of initiative (a higher-initiative mover does not block a lower-initiative mover's path within the same phase — walls still block per D-174, but mover-vs-mover collision is not re-checked mid-phase; if two units' destinations overlap, the later-in-initiative unit's move is clipped by the earlier unit's *starting* footprint, not its new footprint — this is the intentional simplification that makes simultaneous tractable, and it matches Combat_Resolver_5's *phase-level* determinism).
* **Ranged vs melee.** Both use the same initiative-ordered damage walk; the only difference is the *targeting* check (ranged uses `rangeIncrementsSpanned` + defender `touchAc` window, melee uses `threatenedCells`/`footprintDistanceFt`). Cover/concealment/flanking are computed from *pre-move* positions for the whole phase (so a unit that moves out of cover still benefits from cover for shots exchanged that phase — this is the faithful *simultaneous* reading; a sequential reading would incorrectly make it exposed mid-phase).
* **Squads and units.** Orders exist at both granularities: the GM (or a delegated player) issues per-unit `attack`/`move`/`spell_aoe` orders as today, **and** per-squad orders that fan out to every unit in the squad (same `OrderQueue` shape, keyed by `squadId`). The UI surfaces this as `ArmyWindow` + `SquadWindow` order panels; the engine expands squad orders before the phase walk. No new document type — squads are the existing `Army`/`Unit` grouping with a `squadId` tag the UI already renders.
* **Compatibility.** When `mode !== "simultaneous"` the resolver is byte-identical to today (sequential per-unit loop). The feature is `if (ctx.turnMode === "simultaneous") { simultaneousPath } else { sequentialPath }` at the top of `resolveTurn`, not a fork of the whole module. Existing sequential tests must stay green without modification.
* **Determinism & seeding.** Per-unit RNG substreams are forked from `turnSeed(seed, turnNumber)` by `unitId` *before* any damage is applied, so the initiative-ordered damage walk cannot change any attack's die total. Replaying the same seed + orders yields the same `TurnReport.events` + `SimDelta` hash.

#### Data model (no new collection)

* **`TurnMode` (`src/core/strategic.ts`).** Add `"simultaneous"` to the string union; `TurnEngineState` already carries `mode: TurnMode` so no state shape change.
* **`RulesContext.turnMode?: TurnMode`** (optional, default `"stepwise"`), plumbed from `TurnChannel`/`HostApp` the same way `sceneId`/`grid`/`walls`/`leaderActors` already are — `resolveTurn(pool, units, orders, ctx, rng)` reads `ctx.turnMode`.
* **World setting.** `world-settings.flags.pf1e.strategicSimultaneous?: boolean` (or `settings.flags.core.strategicMode`) — the GM toggle. No actor schema change.
* **No new pool columns.** Model count is already `pool.hp` liveness; initiative values ride the existing `PF1eDerived.initiative` / `profile.initiative` compile chain, not a new column.

#### Implementation steps (order matters)

1. **Mode plumbing (pure, no resolver change).** `src/core/strategic.ts:TurnMode` + `TurnEngineState`, `src/host/turnEngine.ts:turnEngineReduce` (`turn.start` takes `mode`), `src/host/turnChannel.ts` + `src/app/hostBoot.ts` forward `turnMode` into `RulesContext`, `src/ui/armies/SettingsWindow.svelte` slider/checkbox for `strategicSimultaneous` (reuses `worldSettingsOps`).
2. **Simultaneous path (deterministic).** `src/packages/massBattlePf1e.ts:resolveTurn` — add the `if (ctx.turnMode === "simultaneous")` branch: (a) expand squad→unit orders, (b) compute `effectiveInitiative` per attacker unit (`deriveFromDocuments` when a `leaderActor` is bound, else `profile.initiative` / `dexMod + bab` fallback — same priority the sheet's `initiativeRoll` already uses), (c) sort attackers by `effectiveInitiative` + tie-breakers, (d) **movement phase** — compute all anchor translations from starting positions, apply in one batch, rebuild the single post-move `SpatialGrid` once, (e) **combat phase** — walk the sorted attackers, each time reading *current* live model count, rolling its attacks with `rng.fork(unitId)`, emitting its `SimEvent`s, and applying its damage to `pool.sys`/`pool.hp` before the next attacker. Cover/flanking are computed from the *post-move* single grid but with the *attacker's* snapshot of defender positions at phase start for the area check, per the spec. Keep the existing `subPhases` list and `TurnReport.summary` counters — the `events` are now initiative-sorted within each sub-phase.
3. **Report ordering.** `TurnReport.events` for simultaneous are sorted `subPhaseOrder, -effectiveInitiative, attackIndex` so the report reads high-initiative damage first, which is exactly the regime's definition.
4. **Tests + e2e.** `tests/packages/massBattlePf1e.test.ts` + new `tests/packages/massBattleSimultaneous.test.ts` (sequential fixture preserved, simultaneous fixture: two 100-archer units at 30 ft, mutual `attack` orders, unit A init 18 vs unit B init 7, deterministic seed → A kills 20 of B before B fires, so B's `attacksExecuted` is 80% of A's; tie-break fixture with equal initiative; movement-phase fixture where a unit moves out of cover but still benefits from cover for that phase's shots). `e2e/pf1e_mass_battles.spec.ts` add a simultaneous scenario (import both zips, set the toggle, issue squad move + unit attack orders, `turn.advance` → report shows both units moved and both fired, but the `7`-initiative unit's damage is lower and the `TurnReport` event order is `A-shoot, B-shoot` with `B`'s `modelsHit` reflecting the reduced pool). Sequential suite must stay green (`--project=chromium` for the pure report-ordering checks).
5. **[x] Combat_Resolver_5 reconciliation (done 2026-09-14, D-223).** The file landed at `Examples/combat_resolver_5.html` (`main` `14a1a77`). Finding (a): the file has **no subPhase names and no shoot→melee staging** — `stepRound` runs `moveUnits(side)` then a per-soldier `soldierTurn` that interleaves volleys and melee; the whole army acts in one half-round batch. PR #18's "single initiative walk, not shoot→melee staging" is file-faithful; no code change. Finding (b): initiative is army-level and verbatim (B12: `r = d20(b) + cfg.armies[i].init`, RAW tie "highest initiative modifier acts first on a tie", then roster order) — transcribed with the reference line quoted in situ into `massBattlePf1e.ts`'s new `strategicArmyInitiative` tie path (the `rollData.ts:initiativeRoll` named above is `pf1eInitiativeRollSpec`; the mass-battle tie path the wording governs is `meleeUnits`' sort, where it now lives). No second initiative formula was invented.

#### Why this shape

* Reroll/Revert rides the *existing* `OpLog` inverse machinery and the *existing* host-evaluated roll path — it is a one-envelope `inverse(old)+new` on top of the flows that already submit `ledgerOps`, not a new transaction log or a 50-round event store. The 2-round bound is a storage + determinism budget (pruning keeps `messages` from growing without bound, and a stale-target refusal is honest). Player Reroll is a *delegation* of the same host path, not a second client RNG, so the audit (`seedClient`/`seedHost`) stays intact and the GM never loses authority.
* Simultaneous is a *mode* on the existing `TurnEngine`/`RulesContext` contract, not a second engine. It reuses the same `resolvePF1eAttacks` per-unit roller, the same `attackEligibility`/`rangeIncrementsSpanned`/`threatenedCells` geometry, and the same `pool.sys` / `pool.hp` writes — only the *order* of the damage writes changes. This keeps the sequential path byte-identical and makes the feature a 150-line branch, not a fork.
* Both features stay tactical-only vs strategic-only as the user asked: roll ledger is explicitly *non-strategic* (mass battle turns have no per-attack roll cards; they have `TurnReport` summaries), and simultaneous is explicitly *strategic* (tactical PF1e already has a per-combatant initiative queue with delay/ready — simultaneous would contradict it).

#### Acceptance (both must be demoable in one `pnpm build` + `pnpm build:systems` + Chromium e2e)

* F01: `e2e/roll_ledger.spec.ts` — non-strategic game, attack at a dummy → card shows `Initiator —what— Target` with `[[total|formula]]` + dropdown of every modifier with reason, initiator/target links center+outline, Fireball area link draws a fading sphere/cone outline that is gone after `fadeSec+1`, Reroll reapplies damage (old HP → new HP), Revert revives a killed model at pre-card HP, Player Reroll grant shows a button only for that player that rerolls host-evaluated, window closes after `T+3` rounds. `vitest` ledger unit tests cover invert, stale-target, permission and 2-round pruning.
...
* **[x] Remaining `??`/`fake` falls in #17 patched through (D-222):** Manyshot ledger carries real per-arrow `attackTotal` (the `?? 10` fallback fabricated every manyshot total); grapple-pin rolls no die and posts an honest narrative card (pin rides the maintain check — no separate PF1e die); `ChatPanel` renders the ledger card's narrative line (the hit/miss/CRITS + hp audit text four pre-#18 specs assert); a successful firearm reload clears the stale `no shot loaded` refusal; `pf1e_join` asserts the D-219-expanded wire schema (`ammo u8`, `weaponState u8`) instead of the pre-#18 contract.
* **[x] Chromium e2e executed in-environment (D-222 recipe over file://)** — `e2e/roll_ledger.spec.ts` 2/2, `e2e/pending_rolls.spec.ts` 2/2, `e2e/pf1e_mass_battles.spec.ts` (incl. F02 100→80 initiative-ordering) green, **full suite 138/138** after the #17 fixes (four pre-remediation reds — firearm misfire/reload, join, A06b — all root-caused and cured).
* **[x] G-04 Combat_Resolver_5 strategic fidelity mode (D-223, 2026-09-14)** — the reference's parallel per-army execution, movement-as-unit and enveloping logic now exist as opt-in settings so a GM never hand-drives one token per turn: `strategicDoctrine` (un-ordered advance units auto-march on the nearest enemy, halting at reach, and auto-engage on contact — honest order synthesis with `issuedBy:"doctrine"` persisted, explicit orders always win), `strategicEnvelop` (engaged wider fronts wrap excess files to the defender's flank slots, corner-anchored, own-side only — Task 4 step 3 restored, D-130-purged: geometry only, no invented modifiers), `strategicArmyInitiative` (B12 verbatim `d20 + initiativeModifier` per army, totals→modifiers→roster tie chain, army rank becomes the primary damage-order key reproducing the whole-army alpha strike). `tests/packages/strategicDoctrine.test.ts` 8 tests pin march/hold/orders-win/contact/envelop slot geometry/B12 totals+tie+alpha ordering/off-by-default.
* **Open frontier for F-items:** only the cross-browser firefox/webkit acceptance (chromium-only here, explicitly deferred by the user) remains for fully marking F01–F03 landed; the `combat_resolver_5.html` verbatim reconciliation is closed (item 5 above, D-223).
* See D-221, D-222 and D-223 in `DECISIONS.md` for the full rationale, the rejected alternatives (tolerant inverses, offline client RNG, weakened assertions, resolver-surgery IGOUGO, auto-volley), and the six-step e2e recipe (replaces D-020's stale toolchain paths).
...
3. **F01 Tactical Roll Ledger** (after P7 tactical flows exist) + **F02 Simultaneous Strategic** (after F01 + TurnEngine `mode` plumbing). Either may be taken out of order if their pure plumbing (ledger model, `TurnMode` enum) is ready, but do not wire F02's `resolveTurn` branch until the tactical ledger's `ledgerOps` contract is frozen so both reuse the same Op-inverse naming.
4. Split **P8** into strategic fixes, hero sync, analytics/UI and content PR-sized changes. Pull schema collisions/scale conversions forward if they block an earlier consumer. Content can proceed with its owning rule phase rather than waiting for all of P8.
5. Apply **V01–V11 throughout**, then complete the combined multiplayer/scale acceptance. Promote **L** items only by explicit scope decision.
6. ~~Reconcile **Combat_Resolver_5** (`combat_resolver_5.html` — re-attach when available) phase names / damage-order tie-breakers verbatim against F02 before marking it landed.~~ **Done 2026-09-14 (D-223)** — file at `Examples/combat_resolver_5.html`; findings and transcription recorded in §13 item 5 and D-223.

Original I PR labels map to this sequence: PR-A/P0 is already landed; B/P1, C/P2, D/P3, E/P4, F–H/P5–P7, I/P8, F01/F02 as post-P7 features. The old G milestone estimates and M task numbers are historical cross-references, not a second schedule or additional copies of these tasks.
