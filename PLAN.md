...
- [x] Fog: per-user explored texture, downscaled readback → fog.put (§9) — D-075 (erase-into-RT reveal; 128px PNG readback; host stores per user+scene) · **live and persistent since D-250**: Settings → Scene switch + sight range, both shells run the reveal loop (`src/client/fogExploration.ts`), the host persists fog.put per user+scene (`fog` store, `fog.get`/`fog.state` restore) and the world file carries `fog/` · **hides tokens since D-251**: a player's canvas draws only their own tokens plus those in current sight (`fogVisibleTokenIds` → `stage.setTokenVisibility`); the GM's cover is translucent (0.35) and never gates
